...
def _create_criteria(version,criteria):
    clean=[validate_criterion(item) for item in criteria]
...
def update_legal_obligation_draft(version,user,criteria=None,**fields):
    ...
    clean_criteria=[validate_criterion(item) for item in criteria] if criteria is not None else None
...
def _validate_contract(version):
    ...
    for criterion in version.criteria.all():validate_criterion({"dimension":criterion.dimension,"operator":criterion.operator,"values":criterion.values,"note":criterion.note})
    ...
    expected={"source":"BCN/LeyChile","candidate_id":candidate.id,"candidate_key":candidate.candidate_key,"extractor_version":candidate.extraction_run.extractor_version,"modality_hint":candidate.modality_hint,"artifact_id":artifact.id,"artifact_sha256":artifact.content_sha256,"parser_version":article.parse.parser_version,"article_id":article.id,"article_key":article.article_key,"article_number":article.article_number,"article_text_hash":article.text_hash,"source_quote":candidate.source_quote,"source_start":candidate.source_start,"source_end":candidate.source_end,"trigger_text":candidate.trigger_text,"trigger_start":candidate.trigger_start,"trigger_end":candidate.trigger_end}
...
def validate_legal_obligation_version(version,user):
    ...
    _validate_contract(version);version.state="validated";version.validated_by=user;version.validated_at=timezone.now();version.save(update_fields=["state","validated_by","validated_at"]);return version
...
def activate_legal_obligation_version(version,user):
    ...
    old.update(state="obsolete",obsoleted_at=now);version.state="active";version.activated_by=user;version.activated_at=now;version.save(update_fields=["state","activated_by","activated_at"]);return version
...
def obsolete_legal_obligation_version(version,user):
    ...
    version.state="obsolete";version.obsoleted_at=timezone.now();version.save(update_fields=["state","obsoleted_at"]);return version
