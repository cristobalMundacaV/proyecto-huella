...
class LecturaSensor(models.Model):
    ...
    co2e_estimado = models.DecimalField(
...
class DispositivoSensor(models.Model):
    ...
    class AmbitoOperacional(models.TextChoices):
        ENERGIA = "energia", "Energia"
        AGUA = "agua", "Agua"
        ...
        RUIDO = "ruido", "Ruido"
        HIDRICA_SUELO = "hidrica-suelo", "Hidrica y suelo"
    ...
    ambito_operacional = models.CharField(
        max_length=40,
        choices=AmbitoOperacional.choices,
        blank=True,
        db_index=True,
    )

    punto_ambiental = models.ForeignKey(
        PuntoAmbientalOperacional,
        on_delete=models.SET_NULL,
    ...
    def clean(self):
        ...
        if (
            self.obra_id
            and self.punto_ambiental_id
            and self.punto_ambiental.obra_id
            and self.punto_ambiental.obra_id != self.obra_id
        ):
...
class InstalacionSensor(models.Model):
    ...
    sensor = models.ForeignKey(
        DispositivoSensor, on_delete=models.CASCADE, related_name="instalaciones"
    )
...
class CalibracionSensor(models.Model):
    ...
    sensor = models.ForeignKey(
        DispositivoSensor, on_delete=models.CASCADE, related_name="calibraciones"
    )
    ...
    def clean(self):
        ...
        if (
            self.evidencia_id
            and self.evidencia.organizacion_id != self.sensor.organizacion_id
        ):
...
class LecturaSensorV2(models.Model):
    ...
    sensor = models.ForeignKey(
        DispositivoSensor, on_delete=models.PROTECT, related_name="lecturas_v2"
    )
    ...
    calidad_tecnica = models.CharField(
        max_length=30, choices=CalidadTecnica.choices, default=CalidadTecnica.VALIDA
    )
    ...
    def clean(self):
        ...
        if (
            self.actividad_id
            and self.actividad.organizacion_id != self.sensor.organizacion_id
        ):
            raise ValidationError(
...
class RegistroSensor(models.Model):
    ...
    co2e_estimado = models.DecimalField(
        max_digits=14, decimal_places=3, null=True, blank=True, editable=False
    )
