...
from .models import (AccionMejoraAmbiental, AlcanceProblematica,
                     IndicadorProblematica, Obra, Organizacion,
...
def problematicas(request, organizacion_id):
    ...
    if request.method == "GET":
        ...
        work = _requested_work(request, org)
        if work is not None:
            rows = rows.filter(obra=work)
        return Response(ProblematicaAmbientalSerializer(rows, many=True).data)
...
def problematica_transition(request, organizacion_id, problematica_id):
    org = get_object_or_404(Organizacion, organizacion_id=organizacion_id)
    try:
        problem = transition_problem(_problem(org, problematica_id, _requested_work(request, org)), request.data.get("estado", ""), user=request.user, detail=request.data.get("detalle", ""))
        return Response(ProblematicaAmbientalSerializer(problem).data)
...
def problematica_action_implement(request, organizacion_id, problematica_id, action_id):
    org = get_object_or_404(Organizacion, organizacion_id=organizacion_id)
    action = get_object_or_404(AccionMejoraAmbiental, problematica=_problem(org, problematica_id, _requested_work(request, org)), pk=action_id)
    try:
...
def problematica_measure_engine(request, organizacion_id, problematica_id):
    ...
    try:
        ...
        measurement = measure_from_engine(
            _problem(org, problematica_id, _requested_work(request, org)), fecha=parse_date(raw_date) if raw_date else None, user=request.user,
        )
        return Response(MedicionSeguimientoAmbientalSerializer(measurement).data, status=status.HTTP_201_CREATED)
    except DjangoValidationError as exc:
        return _error(exc)
...
def problematica_history(request, organizacion_id, problematica_id):
    org = get_object_or_404(Organizacion, organizacion_id=organizacion_id)
    return Response(HistorialProblematicaAmbientalSerializer(_problem(org, problematica_id, _requested_work(request, org)).historial.all(), many=True).data)
