...
class ModelModularizationContractTests(SimpleTestCase):
    ...
    def test_analytics_registry_contains_no_duplicate_model_labels(self):
        ...
        # AccionAmbiental es una capa de compatibilidad histórica externa.
        # Nuevos dominios pueden agregar modelos: verificar identidad pública
        # y registro, en lugar de congelar un conteo histórico de modelos.
        canonical_models = [
            model for model in registered if model is not AccionAmbiental
        ]
        self.assertTrue(canonical_models)
        for model in canonical_models:
            with self.subTest(model=model.__name__):
                self.assertIs(getattr(public_models, model.__name__, None), model)
