...
class ContextGateway:
    ...
    def problem(self, problem, organization):
        ...
        if problem.obra_id:
            problem = ProblematicaAmbiental.objects.select_related("obra").get(
                pk=problem.pk
            )
        ...
        for link in problem.indicadores_v2.select_related("indicador"):
            values = list(
                link.indicador.valores.order_by("-periodo_fin", "-version")[:2]
            )
            comparison = (
                compare_values(link.indicador, values[0].valor, values[1].valor)
                if len(values) > 1
                else {"estado": "sin_base"}
            )
            indicators.append(
                {
                    "id": link.indicador_id,
                    "codigo": link.indicador.codigo,
                    "rol": link.rol,
                    "direccion": link.direccion_deseada,
                    "meta": link.valor_objetivo,
                    "actual": values[0].valor if values else None,
                    "comparacion": comparison,
                }
            )
        cycles = problem.ciclos_reevaluacion.select_related(
            "accion", "resultado"
        ).order_by("-numero")[:3]
        ...
        return {
            ...
            "references": {
                ...
                "problem": problem.id,
                "work": problem.obra_id,
            },
            "problematica": {
                "titulo": problem.titulo,
                "descripcion": problem.descripcion[:500],
                "categoria": problem.categoria,
                "estado": problem.estado,
                "severidad": problem.nivel_riesgo,
                "objetivo": problem.objetivo_ambiental,
            },
            "obra": (
                {
                    "id": problem.obra_id,
                    "nombre": problem.obra.nombre,
                    "estado": problem.obra.estado,
                    "fecha_inicio": problem.obra.fecha_inicio,
                    "fecha_fin": problem.obra.fecha_fin,
                }
                if problem.obra_id
                else None
            ),
            "alcance": [
                {
                    "unidad": row.unidad_operacional_id,
                    "proceso": row.proceso_operacional_id,
                    "activo": row.activo_operacional_id,
                    "actividad": row.actividad_operacional_id,
                    "indicador": row.indicador_id,
                }
                for row in problem.alcances_v2.all()[:20]
            ],
            "kpis": indicators,
            "ciclos": [
                {
                    "numero": row.numero,
                    "accion": row.accion.titulo,
            ...
            "acciones_probadas": [
                {
                    ...
                    "titulo": row.titulo,
                    "estado": row.estado,
                    "justificacion": row.justificacion[:300],
                }
                for row in problem.acciones.order_by("-created_at")[: self.MAX_HISTORY]
            ],
            ...
            "historial_resumido": [
                {
                    ...
                    "detalle": row.detalle[:200],
                    "fecha": row.created_at,
                }
                for row in problem.historial.order_by("-created_at")[: self.MAX_HISTORY]
            ],
    ...
    def organization_memory(self, organization):
        now = timezone.now()
        rows = MemoriaOrganizacion.objects.filter(
            organizacion=organization, vigente_desde__lte=now
        ).filter(Q(vigente_hasta__isnull=True) | Q(vigente_hasta__gte=now))
        return {
            "context_type": "organization_memory",
            "references": {"organization": organization.organizacion_id},
            "items": [
                {
                    "id": row.id,
                    "tipo": row.tipo,
                ...
                for row in rows.order_by("-created_at")[: self.MAX_MEMORY]
            ],
            "restricciones": self._active_restrictions(organization),
    ...
    def asset(self, asset, organization):
        ...
        return {
            ...
            "activo": {
                ...
                "tipo": asset.tipo,
                "estado": asset.estado,
                "unidad": asset.unidad_operacional_id,
                "proceso": asset.proceso_operacional_id,
            },
            "condiciones_recientes": list(
                asset.condiciones.values(
                    "estado", "timestamp_inicio", "timestamp_fin"
                ).order_by("-timestamp_inicio")[: self.MAX_HISTORY]
            ),
        }
    ...
    def asset_maintenance(self, asset, organization):
        ...
        package["mantenimientos"] = list(
            asset.mantenimientos.values(
                "id",
                "tipo",
                "estado",
                "fecha_programada",
    ...
    def sensor_health(self, sensor, organization):
        ...
        return {
            ...
            "sensor": {
                ...
                "habilitado": sensor.activo,
                "activo_id": sensor.activo_operacional_id,
                "activo": (
                    {
                        "id": asset.id,
                        "codigo": asset.codigo,
            ...
            "ultima_calibracion": (
                {
                    "fecha": calibration.fecha,
                    "resultado": calibration.resultado,
                    "proxima": calibration.fecha_proxima_calibracion,
                }
                if calibration
                else None
            ),
            "lecturas_recientes": list(
                sensor.lecturas_v2.values(
                    "timestamp", "concepto", "unidad", "calidad_tecnica"
                ).order_by("-timestamp")[: self.MAX_HISTORY]
            ),
        }
    ...
    def indicator_history(self, indicator, organization):
        ...
        return {
            "context_type": "indicator_history",
            "references": {
                "organization": organization.organizacion_id,
            ...
            "indicador": {
                ...
                "direccion": indicator.direccion_deseable,
            },
            "serie": list(
                indicator.valores.values(
                    "id", "periodo_inicio", "periodo_fin", "valor", "version"
                ).order_by("-periodo_fin", "-version")[: self.MAX_SERIES]
            ),
    ...
    def evidence(self, evidence, organization):
        ...
        return {
            ...
            "contenido_excluido": ["archivo", "texto_extraido", "metadata_extraccion"],
        }
        ...
        package = {
            ...
            "observaciones": list(
                activity.observaciones.values(
                    ...
                    "fuente__tipo",
                    "estado",
                ).order_by("-timestamp_observacion")[:20]
            ),
        }
        journey = getattr(activity, "viaje", None)
        if journey:
            package["transporte"] = {
                "trayecto": journey.tipo_trayecto,
                "gestion": journey.tipo_gestion,
                "estado_carga": journey.estado_carga,
                "origen": journey.origen_nombre,
                "destino": journey.destino_nombre,
                "metricas": journey_metrics(journey),
                "metodologia_tercerizado": journey.metodologia_tercerizado,
            }
        ...
        if material_event:
            ...
            package["material"] = {
                ...
                "balance": material_balance(
                    organization, material_event.material, lot=material_event.lote
                ),
            }
    ...
    def intervention(self, problem, organization):
        ...
        return {
            ...
            "ciclos": [
                {
                    "numero": cycle.numero,
                    "accion": cycle.accion_id,
                    "base": cycle.snapshot_base_id,
                    "resultado_snapshot": cycle.snapshot_resultado_id,
                    "resultado": cycle.resultado.estado if cycle.resultado_id else None,
                }
                for cycle in problem.ciclos_reevaluacion.select_related(
                    "resultado"
                ).order_by("-numero")[:3]
            ],
        }
    ...
    def _active_restrictions(organization, problem=None):
        now = timezone.now()
        rows = RestriccionContextual.objects.filter(
            organizacion=organization, activa=True, vigente_desde__lte=now
        ).filter(Q(vigente_hasta__isnull=True) | Q(vigente_hasta__gte=now))
        if problem:
