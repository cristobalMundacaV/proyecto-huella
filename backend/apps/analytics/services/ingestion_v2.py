...
def _aliases_for(process):
    ...
    if process.destino_operacional == "transporte":
        aliases.update({"combustible": ("combustible_consumido_l", "L"), "litros": ("combustible_consumido_l", "L")})
    elif process.destino_operacional == "flujo_ambiental" and process.flujo == "combustible_estacionario":
        aliases.update({"combustible": ("combustible_consumido", ""), "litros": ("combustible_consumido", "L")})
    return aliases
...
def _leer_archivo(version):
    ...
    try:
        frame = pd.read_csv(version.archivo) if version.nombre_original.lower().endswith(".csv") else pd.read_excel(version.archivo)
    finally: version.archivo.close()
    ...
    for index, row in frame.iterrows():
        ...
        if any(str(value).strip() for value in raw.values()): rows.append((int(index) + 2, raw))
    return columns, rows
...
def crear_ingesta(organizacion, upload, *, fuente_id=None, fuente_nombre="Fuente de ingesta", evidencia_id=None,
    ...
    if not evidencia:
        evidencia = EvidenciaObra.objects.create(
            organizacion=organizacion, nombre=upload.name, tipo_evidencia=_document_evidence_type(destino_operacional),
            archivo=ContentFile(content, name=upload.name), metadata_extraccion={"checksum_sha256": checksum, "ingesta_v2": True},
        )
    number = (evidencia.versiones.order_by("-version").values_list("version", flat=True).first() or 0) + 1
    version = VersionEvidencia(
        evidencia=evidencia, organizacion=organizacion, version=number, archivo=ContentFile(content, name=upload.name),
        nombre_original=upload.name, tipo_documental=clasificacion_confirmada or suggested, checksum_sha256=checksum,
        metadata_tecnica={"size_bytes": len(content), "clasificacion_sugerida": suggested},
    )
    version.full_clean(); version.save()
    process = ProcesoIngesta(
        organizacion=organizacion, version_evidencia=version, fuente_datos=fuente, tipo_ingesta=tipo_ingesta,
...
def _template_for(process):
    return PlantillaMapeo.objects.filter(
        organizacion=process.organizacion, fuente_datos=process.fuente_datos, activa=True,
        tipo_ingesta=process.tipo_ingesta, destino_operacional=process.destino_operacional, flujo=process.flujo,
    ).first()


def _validate_context(process, context):
    if not isinstance(context, dict): raise ValueError("El contexto debe ser un objeto.")
    references = {
        "obra_id": Obra, "unidad_operacional_id": UnidadOperacional, "proceso_operacional_id": ProcesoOperacional,
        "activo_id": ActivoOperacional, "punto_id": PuntoAmbientalOperacional,
    }
...
def analizar_ingesta(proceso):
    if proceso.registros_extraidos.filter(estado=RegistroExtraido.Estado.PROCESADO).exists():
    ...
    for column in columns:
        normalized = normalize_column(column); target = previous.get(normalized) or _aliases_for(proceso).get(normalized)
...
@transaction.atomic
def guardar_mapeo(proceso, mappings, nombre="Mapeo ambiental", *, destino_operacional=None, flujo=None, contexto=None):
    ...
    if not mappings: raise ValueError("Debe informar al menos un mapeo.")
    destination = destino_operacional or proceso.destino_operacional; selected_flow = flujo if flujo is not None else proceso.flujo
    if destination not in ProcesoIngesta.DestinoOperacional.values: raise ValueError("Destino operacional no soportado.")
    ...
    template = PlantillaMapeo(
        organizacion=proceso.organizacion, fuente_datos=proceso.fuente_datos, nombre=nombre, version=version,
        formato="excel_csv", tipo_ingesta=proceso.tipo_ingesta, destino_operacional=destination, flujo=selected_flow,
    )
    template.full_clean(); template.save()
    for item in mappings:
        if item.get("concepto_normalizado"):
            MapeoColumna.objects.create(
                plantilla=template, columna_origen=item["columna_origen"], columna_normalizada=normalize_column(item["columna_origen"]),
                concepto_normalizado=normalize_column(item["concepto_normalizado"]), unidad_esperada=normalize_unit(item.get("unidad_esperada", "")),
            )
    proceso.plantilla_mapeo = template; proceso.destino_operacional = destination; proceso.flujo = selected_flow
    if contexto is not None:
        _validate_context(proceso, contexto); proceso.contexto_confirmado = contexto
    proceso.full_clean(); proceso.estado = ProcesoIngesta.Estado.LISTO_CONFIRMAR; proceso.save()
    return template


def _mapping_for(process):
    if process.plantilla_mapeo_id:
        return {item.columna_normalizada: (item.concepto_normalizado, item.unidad_esperada) for item in process.plantilla_mapeo.mapeos.all()}
...
def _normalize_row(raw, mapping):
    ...
    for column, value in raw.items():
        ...
        if target:
            concept, unit = target; data[concept] = normalize_value(value, concept); units[concept] = normalize_unit(unit)
    row_unit = normalize_unit(data.get("unidad"))
    if row_unit:
        for concept in data:
            if concept not in {"unidad"} and not units.get(concept): units[concept] = row_unit
    return data, units
...
def _context_suggestions(process, data):
    ...
    for concept, (model, output, fields) in specs.items():
        ...
        if matches.count() == 1: result[output] = matches.first().id
        elif matches.count() > 1: errors.append({"codigo": "contexto_ambiguo", "campo": concept, "detalle": f"Existen múltiples coincidencias para '{value}'."})
        else: errors.append({"codigo": "contexto_no_resuelto", "campo": concept, "detalle": f"No existe una coincidencia para '{value}'."})
...
def _capture_errors(process, data):
    ...
    if process.destino_operacional == "flujo_ambiental" and not process.flujo:
        errors.append({"codigo": "flujo_desconocido", "campo": "flujo", "detalle": "Debe seleccionar el flujo ambiental."})
    return errors


def preview_ingesta(proceso):
    mapping = _mapping_for(proceso); rows = []
    for record in proceso.registros_extraidos.all():
        ...
        record.save(update_fields=["datos_normalizados", "errores", "auto_confirmable", "estado", "updated_at"])
        rows.append({"id": record.id, "numero_fila": record.numero_fila, "datos_originales": record.datos_originales,
                     "datos_normalizados": record.datos_normalizados, "destino": proceso.destino_operacional,
                     "flujo": proceso.flujo, "actividad": {"tipo": proceso.flujo or proceso.destino_operacional,
                     "observaciones": [key for key, value in normalized.items() if value not in (None, "")]},
                     "contexto": {**suggestions, **proceso.contexto_confirmado}, "estado": "lista" if not errors else "requiere_revision",
...
def _structured_error(exc):
    ...
    return {"codigo": parts[0] if parts else "error_procesamiento", "campo": parts[1] if len(parts) > 1 else "registro",
...
def confirmar_ingesta(proceso):
    ...
    if not proceso.plantilla_mapeo_id: raise ValueError("Debe confirmar el mapeo antes de procesar.")
    handler = INGESTION_HANDLERS.get(proceso.destino_operacional)
    if not handler: raise ValueError("No existe un handler registrado para el destino.")
    proceso.estado = ProcesoIngesta.Estado.PROCESANDO; proceso.save(update_fields=["estado", "updated_at"])
    mapping = _mapping_for(proceso); processed = errors = observations = 0
    for record in proceso.registros_extraidos.select_for_update():
        ...
        try:
            with transaction.atomic():
                ...
                if capture_errors: raise ValueError(f"{capture_errors[0]['codigo']}|{capture_errors[0]['campo']}|{capture_errors[0]['detalle']}")
                ...
                record.actividad_creada = activity; record.estado = RegistroExtraido.Estado.PROCESADO
                record.errores = []
                record.resultado_procesamiento = {"actividad_id": activity.id, "especializacion": specialization.__class__.__name__ if specialization else "",
                                                  "especializacion_id": getattr(specialization, "id", None), "observacion_ids": [row.id for row in created_observations.values()]}
                record.procesado_at = timezone.now(); record.save(); processed += 1; observations += len(created_observations)
        except Exception as exc:
            record.estado = RegistroExtraido.Estado.ERROR; record.errores = [_structured_error(exc)]
            record.save(update_fields=["estado", "errores", "updated_at"]); errors += 1
