...
from .models import MapeoColumna, PlantillaMapeo, ProcesoIngesta, RegistroExtraido, VersionEvidencia


class MapeoColumnaSerializer(serializers.ModelSerializer):
    class Meta:
        model = MapeoColumna
        fields = ["id", "columna_origen", "columna_normalizada", "concepto_normalizado", "unidad_esperada"]
...
class VersionEvidenciaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "evidencia", "evidencia_nombre", "version", "nombre_original", "tipo_documental",
                  "checksum_sha256", "estado_procesamiento", "metadata_tecnica", "created_at"]


class ProcesoIngestaSerializer(serializers.ModelSerializer):
    version_evidencia_detalle = VersionEvidenciaSerializer(source="version_evidencia", read_only=True)
    fuente_nombre = serializers.CharField(source="fuente_datos.nombre", read_only=True)
