...
from .models import (CalculoAmbiental, FactorAmbiental, FormulaAmbiental, ImpactoAmbiental,
                     InputCalculoAmbiental, MetodologiaAmbiental, VariableFormula,
...
class FormulaAmbientalSerializer(serializers.ModelSerializer):
    ...
    factor_codigo = serializers.CharField(source="factor_ambiental.codigo", read_only=True)
    class Meta:
        model = FormulaAmbiental
        fields = ["id", "codigo", "tipo", "expresion_legible", "version", "factor_ambiental", "factor_codigo", "variables"]
...
class MetodologiaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "codigo", "nombre", "categoria", "flujo", "descripcion", "organizacion", "activa", "versiones", "created_at", "updated_at"]


class VersionFactorSerializer(serializers.ModelSerializer):
    class Meta:
        model = VersionFactorAmbiental
        fields = ["id", "version", "valor", "fuente", "referencia", "region", "vigencia_desde", "vigencia_hasta", "estado", "created_at"]
...
class FactorAmbientalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "codigo", "nombre", "categoria", "sustancia_impacto", "unidad_entrada", "unidad_resultado", "contexto", "organizacion", "versiones", "created_at"]
...
class InputCalculoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "variable", "variable_clave", "observacion", "observacion_detalle", "valor_utilizado", "unidad", "concepto", "fuente", "evidencia", "version_evidencia"]
    def get_observacion_detalle(self, item):
        return {"id": item.observacion_id, "concepto": item.observacion.concepto, "fuente": item.fuente.nombre}


class CalculoAmbientalSerializer(serializers.ModelSerializer):
    metodologia = serializers.CharField(source="version_metodologia.metodologia.nombre", read_only=True)
    metodologia_version = serializers.IntegerField(source="version_metodologia.version", read_only=True)
    formula_detalle = FormulaAmbientalSerializer(source="formula", read_only=True)
    factor_nombre = serializers.CharField(source="version_factor.factor.nombre", read_only=True)
    factor_version = serializers.IntegerField(source="version_factor.version", read_only=True)
    factor_valor = serializers.DecimalField(source="version_factor.valor", max_digits=20, decimal_places=10, read_only=True)
...
class ImpactoAmbientalSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "actividad", "calculo", "tipo", "categoria", "valor", "unidad", "timestamp", "created_at"]
