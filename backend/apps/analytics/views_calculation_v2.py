...
from .models import (CalculoAmbiental, FactorAmbiental, FormulaAmbiental, ImpactoAmbiental,
                     MetodologiaAmbiental, Organizacion, UsuarioOrganizacion, VariableFormula,
                     VersionMetodologia)
from .serializers_calculation_v2 import (CalculoAmbientalSerializer, FactorAmbientalSerializer,
                                        ...
                                        VariableFormulaSerializer, VersionMetodologiaSerializer)
...
def _org(request, value):
    ...
    allowed = request.user.is_authenticated and (request.user.is_superuser or UsuarioOrganizacion.objects.filter(
        user=request.user, organizacion=org, activo=True,
    ).exists())
...
def _serialize_selection(selection):
    ...
    return {
        ...
        "alternativos": [{"metodo": item["metodo"], "estado": item["estado"]} for item in selection["alternativos"]],
...
def metodologia_detail(request, organizacion_id, metodologia_id):
    ...
    if not formula_data: return Response({"formula": ["Este campo es obligatorio."]}, status=400)
    try: validate_applicability(payload.get("aplicabilidad", {}))
    except DjangoValidationError as exc: return Response({"aplicabilidad": exc.messages}, status=400)
    with transaction.atomic():
        version = VersionMetodologia.objects.create(
            metodologia=item, version=(item.versiones.order_by("-version").values_list("version", flat=True).first() or 0) + 1,
            descripcion_tecnica=payload.get("descripcion_tecnica", ""), fuente_referencia=payload.get("fuente_referencia", ""),
            vigencia_desde=payload.get("vigencia_desde") or None, vigencia_hasta=payload.get("vigencia_hasta") or None,
            aplicabilidad=payload.get("aplicabilidad", {}), prioridad=payload.get("prioridad", 100),
            requiere_revision_profesional=payload.get("requiere_revision_profesional", False),
            tipo_resultado=payload.get("tipo_resultado", "emision"),
        )
        factor = get_object_or_404(FactorAmbiental.objects.filter(Q(organizacion=org) | Q(organizacion__isnull=True)), id=formula_data.get("factor_ambiental"))
        formula = FormulaAmbiental.objects.create(version_metodologia=version, factor_ambiental=factor,
            codigo=formula_data.get("codigo", f"formula-{item.codigo}-v{version.version}"), tipo=formula_data.get("tipo"),
            expresion_legible=formula_data.get("expresion_legible", ""), version=formula_data.get("version", 1))
        for row in formula_data.get("variables", []):
            serializer = VariableFormulaSerializer(data=row); serializer.is_valid(raise_exception=True); serializer.save(formula=formula)
...
def metodologia_variables(request, organizacion_id, metodologia_id, version_id, variable_id=None):
    ...
    if request.method == "POST":
        serializer = VariableFormulaSerializer(data=request.data); serializer.is_valid(raise_exception=True); serializer.save(formula=version.formula)
        return Response(serializer.data, status=201)
    variable = get_object_or_404(version.formula.variables, id=variable_id)
    if request.method == "DELETE": variable.delete(); return Response(status=204)
    serializer = VariableFormulaSerializer(variable, data=request.data, partial=True); serializer.is_valid(raise_exception=True); serializer.save()
...
def elegibilidad_actividad(request, organizacion_id, actividad_id):
    org = _org(request, organizacion_id)
    if not org: return Response({"detail": "Recurso no encontrado."}, status=404)
    return Response(_serialize_selection(select_methodology(_activity(org, actividad_id))))
...
def calcular_actividad(request, organizacion_id, actividad_id):
    ...
    try: calculation, selection = calculate_activity(activity, result_context=request.data.get("contexto_resultado"))
    except (ValueError, DjangoValidationError) as exc:
        detail = exc.messages if isinstance(exc, DjangoValidationError) else str(exc)
        return Response({"error": detail, "elegibilidad": _serialize_selection(select_methodology(activity))}, status=400)
...
def calculo_recalculate(request, organizacion_id, calculo_id):
    ...
    try: new, selection = recalculate(calculation, request.data.get("motivo", ""), result_context=request.data.get("contexto_resultado"))
    except (ValueError, DjangoValidationError) as exc:
        return Response({"detail": exc.messages if isinstance(exc, DjangoValidationError) else str(exc)}, status=400)
    return Response({"calculo": CalculoAmbientalSerializer(new).data, "seleccion": _serialize_selection(selection)}, status=201)
...
def impactos_ambientales(request, organizacion_id):
    org = _org(request, organizacion_id)
    if not org: return Response({"detail": "Recurso no encontrado."}, status=404)
    return Response(ImpactoAmbientalSerializer(org.impactos_ambientales_v2.all(), many=True).data)
