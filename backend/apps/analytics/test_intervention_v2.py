...
class InterventionV2Tests(APITestCase):
    def setUp(self):
        ...
        self.unit = UnidadOperacional.objects.create(
            organizacion=self.org, nombre="Logistica"
        )
        ...
        self.asset = ActivoOperacional.objects.create(
            organizacion=self.org, codigo="CAM-7", nombre="Camion 7"
        )
        self.work_a = Obra.objects.create(
            organizacion=self.org, nombre="Obra A", fecha_inicio=date(2026, 1, 1)
        )
        self.work_b = Obra.objects.create(
            organizacion=self.org, nombre="Obra B", fecha_inicio=date(2026, 1, 1)
        )
        self.foreign_work = Obra.objects.create(
            organizacion=self.other, nombre="Obra ajena", fecha_inicio=date(2026, 1, 1)
        )
    ...
    def problem(self, title="Alta intensidad de emisiones en transporte"):
        return ProblematicaAmbiental.objects.create(
            ...
            titulo=title,
            descripcion="Intensidad superior a objetivo",
            categoria="transporte",
            indicador="intensidad",
            unidad_indicador="kgCO2e/t",
            valor_inicial=Decimal("10"),
            objetivo_meta=Decimal("9"),
            fecha_deteccion=date(2026, 1, 1),
            estado="detectada",
            objetivo_ambiental="Reducir intensidad sin ocultar emisiones absolutas",
        )
    ...
    def value(self, indicator, value, month=1, version=1):
        return ValorIndicador.objects.create(
            indicador=indicator,
            periodo_inicio=date(2026, month, 1),
            periodo_fin=date(2026, month, 28),
            valor=value,
            unidad=indicator.unidad,
            fuente_calculo=f"test-v{version}",
            version=version,
        )
    ...
    def test_crear_problematica_alcance_compuesto_y_kpi_principal(self):
        ...
        self.assertEqual(
            (
                scope.unidad_operacional,
                scope.proceso_operacional,
                scope.activo_operacional,
            ),
            (self.unit, self.process, self.asset),
        )
    ...
    def test_alcance_e_indicador_cross_tenant_bloqueados_por_api(self):
        ...
        foreign_indicator = IndicadorAmbiental.objects.create(
        ...
        self.assertEqual(
            self.client.post(
                f"{self.base}/problematicas/{problem.id}/alcance/",
                {"unidad_operacional": foreign_unit.id},
                format="json",
            ).status_code,
            400,
        )
        self.assertEqual(
            self.client.post(
                f"{self.base}/problematicas/{problem.id}/indicadores/",
                {
                    "indicador": foreign_indicator.id,
                    "direccion_deseada": "menor_es_mejor",
                },
                format="json",
            ).status_code,
    ...
    def test_seleccionar_crea_base_e_iniciar_congela_scope(self):
        ...
        cycle = select_action(action, self.user)
        base = cycle.snapshot_base
        self.assertFalse(base.congelado)
        self.assertEqual(base.valores.get().valor, Decimal("10"))
        with self.assertRaises(ValidationError):
            start_action(action, confirmed=False, user=self.user)
        start_action(action, confirmed=True, user=self.user)
        base.refresh_from_db()
        self.assertTrue(base.congelado)
        frozen_scope = base.alcance_congelado.copy()
        AlcanceProblematica.objects.create(
            problematica=problem,
            activo_operacional=ActivoOperacional.objects.create(
                organizacion=self.org, codigo="NEW", nombre="Nuevo"
            ),
        )
        base.refresh_from_db()
        self.assertEqual(base.alcance_congelado, frozen_scope)
        value = base.valores.get()
    ...
    def test_seguimiento_registra_kpi_y_valor_existente(self):
        ...
        start_action(action, confirmed=True)
        tracked = self.value(indicator, Decimal("9.2"), month=2)
        response = self.client.post(
            f"{self.base}/problematicas/{problem.id}/seguimientos/",
            {
                "accion": action.id,
                "fecha": timezone.localdate().isoformat(),
                "indicador_v2": indicator.id,
                "valor_indicador": tracked.id,
                "valor": "9.2",
                "unidad": "kgCO2e/t",
                "fuente": "serie_indicador",
                "referencia": "mes-2",
            },
            format="json",
        )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data["valor_indicador"], tracked.id)

    def test_caso_obligatorio_resultado_positivo_menos_16_por_ciento(self):
        problem, indicator, _, action = self.prepare()
        cycle = select_action(action)
        start_action(action, confirmed=True)
        self.value(indicator, Decimal("8.4"), month=2)
        result = evaluate_intervention(problem)
        self.assertEqual(result.estado, "positiva")
        self.assertEqual(result.snapshot_base, cycle.snapshot_base)
        self.assertEqual(
    ...
    def test_resultado_negativo_se_conserva(self):
        ...
        result = evaluate_intervention(problem)
        self.assertEqual(result.estado, "negativa")
        self.assertTrue(ResultadoIntervencion.objects.filter(pk=result.pk).exists())
        self.assertEqual(
            Decimal(result.metricas_comparadas[0]["porcentaje"]), Decimal("8.00")
        )

    def test_inconclusa_y_solo_kpis_asociados_participan(self):
        problem = self.problem()
        indicator = self.indicator()
        unrelated = self.indicator("unrelated")
        ...
        action = AccionMejoraAmbiental.objects.create(
            problematica=problem, titulo="Sin datos", descripcion="Prueba"
        )
        select_action(action)
        start_action(action, confirmed=True)
        result = evaluate_intervention(problem)
        self.assertEqual(result.estado, "inconclusa")
        self.assertEqual(result.metricas_comparadas, [])

    def test_segundo_ciclo_no_sobrescribe_primero(self):
        problem, indicator, _, action = self.prepare()
        first = select_action(action)
        start_action(action, confirmed=True)
        self.value(indicator, Decimal("10.8"), month=2)
        first_result = evaluate_intervention(problem)
        second = select_action(action)
        self.assertEqual(second.numero, 2)
        self.assertNotEqual(first.snapshot_base_id, second.snapshot_base_id)
        first_result.refresh_from_db()
        self.assertEqual(first_result.estado, "negativa")
    ...
    def test_maximo_tres_ciclos_cuarto_bloqueado_y_escalamiento(self):
        ...
        self.assertEqual(escalated.estado, "escalada_profesional")
        self.assertTrue(escalated.requiere_evaluacion_profesional)
    ...
    def test_cambio_meta_exige_justificacion_y_genera_historial(self):
        ...
        with self.assertRaises(ValidationError):
            change_target(link, Decimal("8"), "", "ajuste", self.user)
        history = change_target(
            link,
            Decimal("8"),
            "Nueva exigencia tecnica",
            "Revision normativa",
            self.user,
        )
        link.refresh_from_db()
        self.assertEqual(link.valor_objetivo, Decimal("8"))
        self.assertEqual(history.problematica, problem)

    def test_endpoints_workspace_y_tenant_isolation(self):
        problem, _, _, action = self.prepare()
        selected = self.client.post(
            f"{self.base}/problematicas/{problem.id}/acciones/{action.id}/seleccionar/",
            {},
            format="json",
        )
        ...
        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/{problem.id}/snapshot-base/"
            ).status_code,
            200,
        )
        self.assertEqual(
            self.client.post(
                f"{self.base}/problematicas/{problem.id}/acciones/{action.id}/iniciar/",
                {"confirmado": True},
                format="json",
            ).status_code,
            200,
        ...
        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/{foreign.id}/ciclos/"
            ).status_code,
            404,
        )

    def test_api_problematicas_scope_obra_listado_detalle_y_organizacional(self):
        problem_a = self.problem("Problema A")
        problem_a.obra = self.work_a
        problem_a.save()
        problem_b = self.problem("Problema B")
        problem_b.obra = self.work_b
        problem_b.save()
        ...
        self.assertCountEqual(
            [row["id"] for row in rows_org.data],
            [problem_a.id, problem_b.id, no_work.id],
        )
        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/{problem_a.id}/?obra={self.work_a.id}"
            ).status_code,
            200,
        )
        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/{problem_b.id}/?obra={self.work_a.id}"
            ).status_code,
            404,
        )
        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/{no_work.id}/?obra={self.work_a.id}"
            ).status_code,
            404,
        )
    ...
    def test_creacion_problematica_conserva_validacion_tenant_de_obra(self):
        payload = {
            "titulo": "Problema nuevo",
            "descripcion": "Detectado en obra",
            "categoria": "energia",
            "indicador": "consumo",
            "unidad_indicador": "kWh",
            "valor_inicial": "10",
            "objetivo_meta": "8",
            "fecha_deteccion": "2026-01-02",
            "obra": self.work_a.id,
        }
        ...
        foreign = self.client.post(
            f"{self.base}/problematicas/", payload, format="json"
        )
    ...
    def test_api_scope_obra_rechaza_cross_tenant_y_cross_work_en_hijos(self):
        ...
        problem_b.obra = self.work_b
        problem_b.save()
        action_b = AccionMejoraAmbiental.objects.create(
            problematica=problem_b, titulo="Accion B", descripcion="Otra obra"
        )
        wrong_scope = f"?obra={self.work_a.id}"

        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/?obra={self.foreign_work.id}"
            ).status_code,
            404,
        )
        self.assertEqual(
            self.client.get(
                f"{self.base}/problematicas/{problem_a.id}/?obra={self.foreign_work.id}"
            ).status_code,
            404,
        )
        for suffix in (
            ...
            response = self.client.get(
                f"{self.base}/problematicas/{problem_b.id}/{suffix}{wrong_scope}"
            )
            self.assertEqual(response.status_code, 404, suffix)
            self.assertEqual(response.data["detail"], "Recurso no encontrado.", suffix)
        self.assertEqual(
            self.client.post(
                f"{self.base}/problematicas/{problem_b.id}/acciones/{action_b.id}/seleccionar/{wrong_scope}",
                {},
                format="json",
            ).status_code,
            404,
    ...
    def test_snapshot_base_distingue_ausencia_de_base_y_scope_incorrecto(self):
        problem_a = self.problem("Sin base")
        problem_a.obra = self.work_a
        problem_a.save()
        problem_b = self.problem("Otra obra")
        problem_b.obra = self.work_b
        problem_b.save()
        own = self.client.get(
            f"{self.base}/problematicas/{problem_a.id}/snapshot-base/?obra={self.work_a.id}"
        )
        foreign = self.client.get(
            f"{self.base}/problematicas/{problem_b.id}/snapshot-base/?obra={self.work_a.id}"
        )
    ...
    def test_problematicas_requieren_membresia_tenant(
        self,
    ):
        ...
        response = client.get(f"{self.base}/problematicas/")
    ...
    def test_creacion_work_scoped_impone_obra_del_contexto(
        self,
    ):
        payload = {
            "titulo": "Problema scope",
            "descripcion": "Problema detectado",
            "categoria": "energia",
            "indicador": "consumo_energia",
            "unidad_indicador": "kWh",
            "valor_inicial": "10",
            "objetivo_meta": "8",
            "fecha_deteccion": "2026-01-02",
        ...
        response = self.client.post(
            f"{self.base}/problematicas/" f"?obra={self.work_a.id}",
            payload,
            format="json",
        )
        ...
        problem = ProblematicaAmbiental.objects.get(pk=response.data["id"])
    ...
    def test_creacion_work_scoped_rechaza_otra_obra(
        self,
    ):
        payload = {
            "titulo": "Problema incorrecto",
            "descripcion": "Scope invalido",
            "categoria": "energia",
            "indicador": "consumo_energia",
            "unidad_indicador": "kWh",
            "valor_inicial": "10",
    ...
    def test_indicador_de_otra_obra_no_se_asocia_a_problema(
        self,
    ):
        problem = self.problem("Problema obra A")

        problem.obra = self.work_a
        problem.save()
        ...
        response = self.client.post(
            f"{self.base}/problematicas/"
            f"{problem.id}/indicadores/"
            f"?obra={self.work_a.id}",
            {
    ...
    def test_actividad_de_otra_obra_no_entra_al_alcance(
        self,
    ):
        problem = self.problem("Problema scope actividad")
        ...
        response = self.client.post(
            f"{self.base}/problematicas/"
            f"{problem.id}/alcance/"
            f"?obra={self.work_a.id}",
            {
                "actividad_operacional": activity.id,
            },
            format="json",
        )
