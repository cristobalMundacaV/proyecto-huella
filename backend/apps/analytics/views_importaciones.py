...
CATEGORY_KEYWORDS = [
    ...
    (
        "Energia",
        ["electricidad", "generador", "diesel", "energia", "kwh", "combustible"],
    ),
    ("Agua", ["agua", "hidrico"]),
...
def parse_organizaciones(raw_rows):
    ...
    for row_number, raw in raw_rows:
        ...
        rows.append(
            row_result(
                row_number, data, errors, warnings, "actualizar" if exists else "crear"
            )
        )
    return rows
    ...
    for row_number, raw in raw_rows:
        ...
        data = {
            ...
            "categoria": normalize_category(
                get(raw, "Categoría", "Categoria"), activity
            ),
            "unidad": get(
                raw, "Unidad", "Etapa", "Unidad de medida", "Tipo de consumo"
            ),
            "factor_emision": get(
                raw, "Factor de Emisión", "Factor de emision", "Factor"
            ),
            "fuente": get(raw, "Fuente", "Fuente dato", "Referencia")
            or "Carga importada",
        ...
        rows.append(
            row_result(
                row_number, data, errors, [], "actualizar" if exists else "crear"
            )
        )
    return rows
...
def parse_obras(raw_rows, organizacion=None, known_stage_ids=None):
    ...
    for row_number, raw in raw_rows:
        ...
        data = {
            ...
            "tipo_proyecto": tipo,
            "superficie_m2": get(
                raw, "Cantidad base", "Superficie", "Superficie m2", "m2"
            ),
            "origen": get(
                raw, "Ubicación / origen", "Ubicacion", "Ubicación", "Origen"
            ),
            "ubicacion": get(
                raw, "Ubicación / origen", "Ubicacion", "Ubicación", "Origen"
            ),
        ...
        if (
            data["etapa_id"]
            and data["etapa_id"] not in known_stage_ids
            and not EtapaObra.objects.filter(etapa_id=data["etapa_id"]).exists()
        ):
...
def parse_registros(
    ...
    for row_number, raw in raw_rows:
        source = get(
            raw, "Fuente", "Fuente de emisión", "Fuente de emision", "Actividad"
        )
        ...
        factor_value = (
            get(raw, "Factor", "Factor de emisión", "Factor de emision")
            or factor_from_file
            or (str(factor_obj.factor_emision) if factor_obj else "")
        )
        data = {
            ...
            "categoria": normalize_category(
                get(raw, "Categoría", "Categoria")
                or (factor_obj.categoria if factor_obj else ""),
                source,
            ),
        ...
        if dec(data["factor_emision"], None) is None:
            errors.append(
                "No se encontró factor de emisión válido para la fuente/unidad."
            )
        if not as_date(data["fecha"]):
            errors.append("La fecha debe ser válida.")
        if (
            data["codigo_obra"]
            and data["codigo_obra"] not in known_work_codes
...
def parse_evidencias(raw_rows, organizacion=None, known_work_codes=None):
    ...
    for row_number, raw in raw_rows:
        data = {
            "codigo_obra": get(raw, "Código de obra", "Codigo de obra", "codigo_obra"),
            "etapa_id": get(raw, "ID Etapa", "Etapa ID", "id_etapa"),
            "tipo_evidencia": norm(get(raw, "Tipo evidencia", "Tipo de evidencia"))
            or "otro",
            "nombre": get(raw, "Nombre", "Documento", "Archivo")
            or "Evidencia importada",
            "fecha_documento": get(raw, "Fecha", "Fecha documento"),
            "estado_documental": norm(get(raw, "Estado", "Estado documental"))
            or "pendiente",
            "archivo": get(raw, "Archivo", "Archivo referencia")
            or "documento_importado.txt",
            "observaciones": get(raw, "Observaciones"),
        }
        errors = []
        if (
...
def summary(rows, kind):
    ...
    errors = total - valid
    updates = len(
        [row for row in valid_rows(rows) if row.get("db_action") == "actualizar"]
    )
    ...
    if kind in {"organizaciones", "factores"}:
        return {
            ...
            "posibles_creaciones": creates,
            "posibles_actualizaciones": updates,
        }
    if kind == "etapas":
        return {
            "validas": valid,
    ...
    if kind == "obras":
        return {
            "validas": valid,
            "con_error": errors,
            "obras_nuevos": creates,
            "obras_existentes": updates,
            "duplicadas": duplicates,
        }
...
def registros_summary(rows):
    ...
    return {
        ...
        "factores_encontrados": len(
            [row for row in valid if row["data"].get("factor_emision")]
        ),
        "factores_faltantes": len(
            [
                row
                for row in rows
                if any("factor" in error.lower() for error in row.get("errors", []))
            ]
        ),
        "obras_encontrados": len(
            [row for row in valid if row["data"].get("codigo_obra")]
        ),
...
def preview_payload(kind, rows):
    payload = {
        "rows": rows,
        "summary": (
            registros_summary(rows) if kind == "registros" else summary(rows, kind)
        ),
    }
...
def parse_columns_from_request(request):
    raw_columns = (
        request.data.get("columns") or request.query_params.get("columns") or ""
    )

    if isinstance(raw_columns, list):
        return [norm(column) for column in raw_columns if clean(column)]

    try:
        parsed = json.loads(raw_columns)
        if isinstance(parsed, list):
            return [norm(column) for column in parsed if clean(column)]
    except (TypeError, ValueError, json.JSONDecodeError):
        pass

    return [norm(column) for column in str(raw_columns).split(",") if clean(column)]


def has_operational_amount(data):
    amount_fields = [
        "cantidad",
        "consumo_kwh",
        "energia_kwh",
    ...
    return any(clean(data.get(field)) for field in amount_fields)


def generic_import_summary(rows):
    total = len(rows)
    valid = len(valid_rows(rows))
    errors = total - valid
    warnings = len([row for row in rows if row.get("warnings")])
    ...
    return {
        ...
        "factores_encontrados": len(
            [
                row
                for row in rows
                if dec(row.get("data", {}).get("factor_emision"), None) is not None
            ]
        ),
        "factores_faltantes": len(
            [
                row
                for row in rows
                if dec(row.get("data", {}).get("factor_emision"), None) is None
            ]
        ),
        "duplicados": 0,
    }
...
def parse_generic_import(upload, columns):
    if not columns:
        raise ValueError("No se recibieron columnas para validar la plantilla.")

    raw_rows = read_rows(upload)
    rows = []

    for row_number, raw in raw_rows:
        data = {column: clean(raw.get(norm(column), "")) for column in columns}
        errors = []
        warnings = []

        missing_columns = [column for column in columns if norm(column) not in raw]
        if missing_columns:
            errors.extend([f"Falta columna {column}" for column in missing_columns])

        if not has_operational_amount(data):
            errors.append("Falta cantidad operacional.")

        if "factor_emision" in data and dec(data.get("factor_emision"), None) is None:
            warnings.append("Sin factor de emisión.")

        rows.append(row_result(row_number, data, errors, warnings))
...
def importacion_generica_preview(request):
    try:
        columns = parse_columns_from_request(request)
        rows = parse_generic_import(get_upload(request), columns)
        payload = {
            "rows": rows,
            "summary": generic_import_summary(rows),
...
@api_view(["GET"])
def plantilla_importacion_generica(request):
    columns = parse_columns_from_request(request)
    filename = request.query_params.get("filename") or "plantilla_importacion.xlsx"

    if not columns:
        return Response(
            {"error": "Debes indicar columnas para generar la plantilla."},
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(columns=columns).to_excel(writer, sheet_name="datos", index=False)
        pd.DataFrame(
            [
                {
                    "instruccion": "Completa los datos en la hoja 'datos'. No cambies los nombres de las columnas.",
                },
                {
                    "instruccion": "Puedes dejar factor_emision vacío si todavía no tienes factor validado.",
                },
                {
                    "instruccion": "El sistema validará filas, cantidades y campos faltantes antes de confirmar.",
                },
            ]
    ...
    for row in valid_rows(rows):
        ...
        _, was_created = Obra.objects.update_or_create(
            ...
            defaults={
                ...
                "fecha_inicio": as_date(data.get("fecha_inicio"))
                or as_date(data.get("fecha")),
                "superficie_m2": dec(data.get("superficie_m2"), None),
                "ubicacion": data.get("ubicacion") or data.get("origen", ""),
                "region": data.get("region", ""),
                "comuna": data.get("comuna", ""),
                "estado": data.get("estado") or "en_ejecucion",
                "descripcion": data.get("descripcion", ""),
            },
        )
        created += int(was_created)
...
def save_evidencias(rows, organizacion=None):
    ...
    for row in valid_rows(rows):
        ...
        evidencia = EvidenciaObra(
            ...
            estado_documental=data.get("estado_documental") or "pendiente",
            fecha_documento=as_date(data.get("fecha_documento")),
            nombre=data.get("nombre") or "Evidencia importada",
            observaciones=data.get("observaciones", ""),
        )
        filename = data.get("archivo") or f"evidencia_importada_{uuid.uuid4().hex}.txt"
        evidencia.archivo.save(
            filename,
            ContentFile("Archivo referencial creado por importación de datos."),
            save=True,
        )
        created += 1
    return {
...
def plantilla_importacion_construccion(request):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        ).to_excel(writer, sheet_name="factores", index=False)
        pd.DataFrame(
            columns=[
                "ID Etapa",
                "ID organizacion",
        ...
        ).to_excel(writer, sheet_name="etapas", index=False)
        pd.DataFrame(
            columns=[
                "Código de obra",
                "ID organizacion",
        ...
        ).to_excel(writer, sheet_name="obras", index=False)
        pd.DataFrame(
            columns=[
                "ID Registro",
                "Código de obra",
        ...
        ).to_excel(writer, sheet_name="registros", index=False)
        pd.DataFrame(
            columns=[
                "Código de obra",
                "ID Etapa",
    ...
    response["Content-Disposition"] = (
        'attachment; filename="plantilla_importacion_construccion.xlsx"'
    )
    return response
...
def preview_complete_payload(upload):
    ...
    factor_map = {
        (norm(row["data"].get("fuente_emision")), norm(row["data"].get("unidad"))): row[
            "data"
        ].get("factor_emision")
    ...
    dates = [
        ...
        if as_date(row["data"].get("fecha"))
    ]
    emissions = sum(
        float(
            dec(row["data"].get("cantidad"), Decimal("0"))
            * dec(row["data"].get("factor_emision"), Decimal("0"))
        )
        for row in valid_rows(registros)
    )
    payload = {
        ...
        "factores": {
            "rows": factores,
            "total": len(factores),
            "validas": len(valid_rows(factores)),
            "errores": len(factores) - len(valid_rows(factores)),
        },
        "etapas": {
            "rows": etapas,
            "total": len(etapas),
            "validas": len(valid_rows(etapas)),
            "errores": len(etapas) - len(valid_rows(etapas)),
        },
        "obras": {
            "rows": obras,
            "total": len(obras),
            "validos": len(valid_rows(obras)),
            "errores": len(obras) - len(valid_rows(obras)),
        },
        "registros_emision": {
            "rows": registros,
            "total": len(registros),
            "validas": len(valid_rows(registros)),
            "errores": len(registros) - len(valid_rows(registros)),
            "factores_faltantes": registros_summary(registros)["factores_faltantes"],
        },
        "evidencias": {
            "rows": evidencias,
            "total": len(evidencias),
            "validas": len(valid_rows(evidencias)),
            "errores": len(evidencias) - len(valid_rows(evidencias)),
        },
        ...
        "resumen": {
            ...
            "emisiones_estimadas_kg_co2e": emissions,
            "alertas": [],
        },
    }
...
def importacion_completa_confirm(request):
    ...
    if not batch:
        return Response(
            {
                "error": "La previsualización expiró. Carga nuevamente el archivo antes de confirmar."
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
