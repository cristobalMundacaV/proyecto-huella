...
class RutaOperacional(models.Model):
    organizacion = models.ForeignKey(Organizacion, on_delete=models.CASCADE, related_name="rutas_operacionales")
    codigo = models.CharField(max_length=100)
    ...
    distancia_planificada = models.DecimalField(max_digits=16, decimal_places=3, null=True, blank=True)
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["organizacion", "codigo"], name="unique_ruta_operacional_codigo_org")]
...
class ViajeOperacional(models.Model):
    ...
    organizacion = models.ForeignKey(Organizacion, on_delete=models.CASCADE, related_name="viajes_operacionales")
    actividad = models.OneToOneField(ActividadOperacional, on_delete=models.CASCADE, related_name="viaje")
    codigo = models.CharField(max_length=100)
    vehiculo = models.ForeignKey(Vehiculo, on_delete=models.PROTECT, related_name="viajes")
    ruta = models.ForeignKey(RutaOperacional, on_delete=models.SET_NULL, null=True, blank=True, related_name="viajes")
    origen_nombre = models.CharField(max_length=240)
    origen_referencia = models.CharField(max_length=180, blank=True)
    origen_coordenadas = models.JSONField(default=dict, blank=True)
    destino_nombre = models.CharField(max_length=240)
    ...
    observacion_distancia = models.ForeignKey(Observacion, on_delete=models.PROTECT, null=True, blank=True, related_name="viajes_distancia_seleccionada")
    observacion_carga = models.ForeignKey(Observacion, on_delete=models.PROTECT, null=True, blank=True, related_name="viajes_carga_seleccionada")
    observacion_combustible = models.ForeignKey(Observacion, on_delete=models.PROTECT, null=True, blank=True, related_name="viajes_combustible_seleccionado")
    estado_carga = models.CharField(max_length=25, choices=EstadoCarga.choices, default=EstadoCarga.DESCONOCIDO)
    tipo_trayecto = models.CharField(max_length=15, choices=TipoTrayecto.choices, default=TipoTrayecto.IDA)
    tipo_gestion = models.CharField(max_length=15, choices=TipoGestion.choices, default=TipoGestion.PROPIO)
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["organizacion", "codigo"], name="unique_viaje_operacional_codigo_org")]

    def clean(self):
        from django.core.exceptions import ValidationError
        errors = {}
        if self.actividad_id and (self.actividad.organizacion_id != self.organizacion_id or self.actividad.tipo != ActividadOperacional.Tipo.TRANSPORTE):
            errors["actividad"] = "La actividad debe ser de transporte y pertenecer a la organizacion."
        if self.vehiculo_id and self.vehiculo.activo.organizacion_id != self.organizacion_id:
        ...
        expected = {"observacion_distancia": "distancia_recorrida_km", "observacion_carga": "masa_transportada_t", "observacion_combustible": "combustible_consumido_l"}
        for field, concept in expected.items():
            observation = getattr(self, field, None)
            if observation and (observation.organizacion_id != self.organizacion_id or observation.actividad_id != self.actividad_id or observation.concepto != concept):
...
class MaterialOperacional(models.Model):
    organizacion = models.ForeignKey(Organizacion, on_delete=models.CASCADE, related_name="materiales_operacionales")
    codigo = models.CharField(max_length=100)
    nombre = models.CharField(max_length=180)
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["organizacion", "codigo"], name="unique_material_operacional_codigo_org")]


class LoteMaterial(models.Model):
    organizacion = models.ForeignKey(Organizacion, on_delete=models.CASCADE, related_name="lotes_materiales")
    material = models.ForeignKey(MaterialOperacional, on_delete=models.PROTECT, related_name="lotes")
    ...
    cantidad_inicial = models.DecimalField(max_digits=20, decimal_places=6, null=True, blank=True)
    unidad = models.CharField(max_length=40, blank=True)
    fuente = models.ForeignKey(FuenteDatos, on_delete=models.PROTECT, null=True, blank=True, related_name="lotes_materiales")
    evidencia = models.ForeignKey("EvidenciaObra", on_delete=models.SET_NULL, null=True, blank=True, related_name="lotes_materiales")
    version_evidencia = models.ForeignKey("VersionEvidencia", on_delete=models.SET_NULL, null=True, blank=True, related_name="lotes_materiales")
    metadata = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["organizacion", "codigo"], name="unique_lote_material_codigo_org")]
...
class EventoMaterial(models.Model):
    ...
    organizacion = models.ForeignKey(Organizacion, on_delete=models.CASCADE, related_name="eventos_materiales")
    material = models.ForeignKey(MaterialOperacional, on_delete=models.PROTECT, related_name="eventos")
    lote = models.ForeignKey(LoteMaterial, on_delete=models.SET_NULL, null=True, blank=True, related_name="eventos")
    actividad = models.OneToOneField(ActividadOperacional, on_delete=models.PROTECT, related_name="evento_material")
    evento_origen = models.ForeignKey("self", on_delete=models.SET_NULL, null=True, blank=True, related_name="eventos_derivados")
    tipo = models.CharField(max_length=25, choices=Tipo.choices, db_index=True)
    fecha_hora = models.DateTimeField(db_index=True)
    observacion_cantidad = models.ForeignKey(Observacion, on_delete=models.PROTECT, null=True, blank=True, related_name="eventos_material_cantidad_seleccionada")
    origen = models.CharField(max_length=240, blank=True)
    destino = models.CharField(max_length=240, blank=True)
    obra = models.ForeignKey("Obra", on_delete=models.SET_NULL, null=True, blank=True, related_name="eventos_materiales")
    proceso = models.ForeignKey(ProcesoOperacional, on_delete=models.SET_NULL, null=True, blank=True, related_name="eventos_materiales")
    fuente = models.ForeignKey(FuenteDatos, on_delete=models.PROTECT, null=True, blank=True, related_name="eventos_materiales")
    evidencia = models.ForeignKey("EvidenciaObra", on_delete=models.SET_NULL, null=True, blank=True, related_name="eventos_materiales")
    version_evidencia = models.ForeignKey("VersionEvidencia", on_delete=models.SET_NULL, null=True, blank=True, related_name="eventos_materiales")
    estado = models.CharField(max_length=20, choices=Estado.choices, default=Estado.REGISTRADO, db_index=True)
    metadata = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["organizacion", "material", "fecha_hora"]), models.Index(fields=["organizacion", "lote", "fecha_hora"])]

    def clean(self):
        from django.core.exceptions import ValidationError
        errors = {}
        for field in ("material", "lote", "actividad", "evento_origen", "obra", "proceso", "fuente", "evidencia", "version_evidencia", "observacion_cantidad"):
            value = getattr(self, field, None)
            if value and value.organizacion_id != self.organizacion_id:
                errors[field] = "La referencia debe pertenecer a la misma organizacion."
        ...
        if self.observacion_cantidad_id:
            ...
            if observation.actividad_id != self.actividad_id or observation.concepto != "cantidad_material":
                errors["observacion_cantidad"] = "La observacion debe ser la cantidad de la actividad del evento."
            elif observation.valor_numerico is None or observation.valor_numerico < 0 or not observation.unidad:
                errors["observacion_cantidad"] = "La cantidad seleccionada debe ser numerica, no negativa y tener unidad."
        if self.version_evidencia_id and self.evidencia_id and self.version_evidencia.evidencia_id != self.evidencia_id:
            errors["version_evidencia"] = "La version debe pertenecer a la evidencia asociada."
        if errors:
            raise ValidationError(errors)
...
class ProblematicaAmbiental(models.Model):
    ...
    responsable_usuario = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name="problematicas_responsable")
...
class AccionMejoraAmbiental(models.Model):
    ...
    responsable_usuario = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name="acciones_mejora_responsable")
    observaciones = models.TextField(blank=True)
...
class MedicionSeguimientoAmbiental(models.Model):
    ...
    indicador_v2 = models.ForeignKey("IndicadorAmbiental", on_delete=models.PROTECT, null=True, blank=True, related_name="mediciones_intervencion")
    valor_indicador = models.ForeignKey("ValorIndicador", on_delete=models.PROTECT, null=True, blank=True, related_name="mediciones_intervencion")
    referencia = models.CharField(max_length=240, blank=True)
    observaciones = models.TextField(blank=True)
    evidencia = models.ForeignKey("EvidenciaObra", on_delete=models.SET_NULL, null=True, blank=True, related_name="mediciones_intervencion")
    metadata = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def clean(self):
        ...
        if self.accion_id and self.accion.problematica_id != self.problematica_id:
            raise ValidationError({"accion": "La accion pertenece a otra problematica."})
        if self.indicador_v2_id and self.indicador_v2.organizacion_id != organization_id:
            raise ValidationError({"indicador_v2": "El indicador pertenece a otra organizacion."})
...
class AlcanceProblematica(models.Model):
    problematica = models.ForeignKey(ProblematicaAmbiental, on_delete=models.CASCADE, related_name="alcances_v2")
    unidad_operacional = models.ForeignKey(UnidadOperacional, on_delete=models.PROTECT, null=True, blank=True, related_name="alcances_problematicas")
    proceso_operacional = models.ForeignKey(ProcesoOperacional, on_delete=models.PROTECT, null=True, blank=True, related_name="alcances_problematicas")
    activo_operacional = models.ForeignKey(ActivoOperacional, on_delete=models.PROTECT, null=True, blank=True, related_name="alcances_problematicas")
    actividad_operacional = models.ForeignKey(ActividadOperacional, on_delete=models.PROTECT, null=True, blank=True, related_name="alcances_problematicas")
    indicador = models.ForeignKey(IndicadorAmbiental, on_delete=models.PROTECT, null=True, blank=True, related_name="alcances_problematicas")
    created_at = models.DateTimeField(auto_now_add=True)

    def clean(self):
        from django.core.exceptions import ValidationError
        organization_id = self.problematica.organizacion_id
        if not any([self.unidad_operacional_id, self.proceso_operacional_id, self.activo_operacional_id, self.actividad_operacional_id, self.indicador_id]):
        ...
        for field in ("unidad_operacional", "proceso_operacional", "activo_operacional", "actividad_operacional", "indicador"):
            ...
            if relation and relation.organizacion_id != organization_id:
                raise ValidationError({field: "La referencia pertenece a otra organizacion."})
...
class IndicadorProblematica(models.Model):
    ...
    problematica = models.ForeignKey(ProblematicaAmbiental, on_delete=models.CASCADE, related_name="indicadores_v2")
    indicador = models.ForeignKey(IndicadorAmbiental, on_delete=models.PROTECT, related_name="problematicas_v2")
    rol = models.CharField(max_length=20, choices=Rol.choices, default=Rol.PRINCIPAL)
    valor_objetivo = models.DecimalField(max_digits=24, decimal_places=10, null=True, blank=True)
    direccion_deseada = models.CharField(max_length=25, choices=IndicadorAmbiental.DireccionDeseable.choices)
    obligatorio = models.BooleanField(default=True)
    orden = models.PositiveIntegerField(default=1)

    class Meta:
        ordering = ["orden", "id"]
        constraints = [models.UniqueConstraint(fields=["problematica", "indicador"], name="unique_indicador_problematica")]

    def clean(self):
        from django.core.exceptions import ValidationError
        if self.indicador_id and self.indicador.organizacion_id != self.problematica.organizacion_id:
            raise ValidationError({"indicador": "El indicador pertenece a otra organizacion."})
...
class SnapshotIntervencion(models.Model):
    ...
    problematica = models.ForeignKey(ProblematicaAmbiental, on_delete=models.PROTECT, related_name="snapshots_intervencion")
    accion = models.ForeignKey(AccionMejoraAmbiental, on_delete=models.PROTECT, related_name="snapshots_intervencion")
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["problematica", "accion", "ciclo", "tipo"], name="unique_snapshot_intervencion_ciclo")]

    def save(self, *args, **kwargs):
        if self.pk and SnapshotIntervencion.objects.filter(pk=self.pk, congelado=True).exists():
            from django.core.exceptions import ValidationError
...
class SnapshotValorIndicador(models.Model):
    snapshot = models.ForeignKey(SnapshotIntervencion, on_delete=models.PROTECT, related_name="valores")
    indicador = models.ForeignKey(IndicadorAmbiental, on_delete=models.PROTECT, related_name="valores_snapshot")
    valor = models.DecimalField(max_digits=24, decimal_places=10)
    unidad = models.CharField(max_length=60)
    periodo_inicio = models.DateField()
    periodo_fin = models.DateField()
    valor_indicador_origen = models.ForeignKey(ValorIndicador, on_delete=models.PROTECT, null=True, blank=True, related_name="snapshots_intervencion")

    class Meta:
        constraints = [models.UniqueConstraint(fields=["snapshot", "indicador"], name="unique_valor_snapshot_indicador")]

    def save(self, *args, **kwargs):
        if self.snapshot.congelado:
            from django.core.exceptions import ValidationError
            raise ValidationError("Los valores de un snapshot congelado son inmutables.")
        super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        if self.snapshot.congelado:
            from django.core.exceptions import ValidationError
            raise ValidationError("Los valores de un snapshot congelado no pueden eliminarse.")
        return super().delete(*args, **kwargs)
...
class ResultadoIntervencion(models.Model):
    ...
    problematica = models.ForeignKey(ProblematicaAmbiental, on_delete=models.PROTECT, related_name="resultados_intervencion")
    accion = models.ForeignKey(AccionMejoraAmbiental, on_delete=models.PROTECT, related_name="resultados_intervencion")
    ciclo = models.PositiveIntegerField()
    snapshot_base = models.ForeignKey(SnapshotIntervencion, on_delete=models.PROTECT, related_name="resultados_como_base")
    snapshot_resultado = models.ForeignKey(SnapshotIntervencion, on_delete=models.PROTECT, related_name="resultados_como_resultado")
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["problematica", "accion", "ciclo"], name="unique_resultado_intervencion_ciclo")]


class CicloReevaluacionProblematica(models.Model):
    problematica = models.ForeignKey(ProblematicaAmbiental, on_delete=models.PROTECT, related_name="ciclos_reevaluacion")
    numero = models.PositiveSmallIntegerField()
    accion = models.ForeignKey(AccionMejoraAmbiental, on_delete=models.PROTECT, related_name="ciclos_reevaluacion")
    snapshot_base = models.ForeignKey(SnapshotIntervencion, on_delete=models.PROTECT, null=True, blank=True, related_name="ciclos_como_base")
    snapshot_resultado = models.ForeignKey(SnapshotIntervencion, on_delete=models.PROTECT, null=True, blank=True, related_name="ciclos_como_resultado")
    resultado = models.ForeignKey(ResultadoIntervencion, on_delete=models.PROTECT, null=True, blank=True, related_name="ciclo_reevaluacion")
    fecha_inicio = models.DateField()
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["problematica", "numero"], name="unique_ciclo_problematica")]

    def clean(self):
        from django.core.exceptions import ValidationError
        if self.numero > 3:
            raise ValidationError({"numero": "No se permite un cuarto ciclo automatico."})
        if self.accion_id and self.accion.problematica_id != self.problematica_id:
            raise ValidationError({"accion": "La accion pertenece a otra problematica."})


class HistorialMetaProblematica(models.Model):
    problematica = models.ForeignKey(ProblematicaAmbiental, on_delete=models.PROTECT, related_name="historial_metas")
    indicador_problematica = models.ForeignKey(IndicadorProblematica, on_delete=models.PROTECT, related_name="historial_metas")
    valor_anterior = models.DecimalField(max_digits=24, decimal_places=10, null=True, blank=True)
    valor_nuevo = models.DecimalField(max_digits=24, decimal_places=10)
    justificacion_tecnica = models.TextField()
    motivo = models.TextField()
    usuario = models.ForeignKey(User, on_delete=models.PROTECT, related_name="cambios_meta_problematica")
