...
def limites_ambientales(request, organizacion_id):
    ...
    return Response(LimiteNormativoAmbientalSerializer(limite).data, status=status.HTTP_201_CREATED)
...
def limite_ambiental_detail(request, organizacion_id, limite_id):
    ...
    if request.method == "DELETE":
        limite.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
    serializer = LimiteNormativoAmbientalSerializer(limite, data=request.data, partial=True)
    serializer.is_valid(raise_exception=True)
    limite = serializer.save()
    return Response(LimiteNormativoAmbientalSerializer(limite).data)
...
def cumplimiento_ambiental_resumen(request, organizacion_id):
    ...
    cumplen = variables.filter(estado_cumplimiento=VariableAmbientalExtraida.EstadoCumplimiento.CUMPLE).count()
    compliance_pct = round((cumplen / total_variables) * 100, 2) if total_variables else 0

    latest_critical = alertas.filter(alertas_abiertas_q, severidad__in=["rojo", "amarillo"]).order_by("-created_at")[:5]
    ...
    return Response(
        {
            ...
            "variables_cumplen": estados_map.get(VariableAmbientalExtraida.EstadoCumplimiento.CUMPLE, 0),
            "variables_alerta": estados_map.get(VariableAmbientalExtraida.EstadoCumplimiento.ALERTA, 0),
            "variables_incumplen": estados_map.get(VariableAmbientalExtraida.EstadoCumplimiento.INCUMPLE, 0),
            "variables_sin_limite": estados_map.get(VariableAmbientalExtraida.EstadoCumplimiento.SIN_LIMITE, 0),
            "alertas_abiertas": alertas.filter(alertas_abiertas_q).count(),
            "alertas_rojas": alertas.filter(alertas_abiertas_q, severidad="rojo").count(),
            "alertas_amarillas": alertas.filter(alertas_abiertas_q, severidad="amarillo").count(),
            "compliance_pct": compliance_pct,
            "critical_alerts": AlertaCumplimientoAmbientalSerializer(latest_critical, many=True).data,
            "recent_documents": DocumentoAmbientalSerializer(recent_documents, many=True).data,
        }
    )
