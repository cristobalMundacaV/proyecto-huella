...
from .services.document_provider import DocumentProviderConfigurationError
...
class DocumentProviderRegistryTests(SimpleTestCase):
    ...
    def test_openai_y_openrouter_producen_el_mismo_document_claims(self):
        ...
        openrouter_client.chat.completions.create.return_value = Mock(
            choices=[Mock(message=Mock(content=json.dumps(PROVIDER_RESULT)))]
        )
    ...
    def test_fallback_se_usa_solo_cuando_esta_configurado(self, openrouter_sdk):
        client = Mock()
        client.chat.completions.create.return_value = Mock(choices=[Mock(message=Mock(content=json.dumps(PROVIDER_RESULT)))])
