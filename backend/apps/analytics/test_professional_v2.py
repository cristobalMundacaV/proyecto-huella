...
class ProfessionalV2Tests(APITestCase):
    def setUp(self):
        self.professional = User.objects.create_user(
            "professional", password="test-pass", first_name="Ana", last_name="Revisora"
        )
        ...
        UsuarioOrganizacion.objects.create(
            user=self.operator, organizacion=self.org, rol="operador"
        )
        self.client.force_login(self.professional)
        self.base = f"/api/organizaciones/{self.org.organizacion_id}"
        self.source = FuenteDatos.objects.create(
            organizacion=self.org, nombre="GPS-CAM-01", tipo="gps"
        )
        ...
        self.distance = Observacion.objects.create(
            organizacion=self.org,
            ...
            estado="validada",
            metodo_captura="instrumental",
        )
        self.mass = Observacion.objects.create(
            organizacion=self.org,
            actividad=self.activity,
            fuente=self.source,
        ...
        factor = FactorAmbiental.objects.create(
            organizacion=self.org,
            codigo="factor-tkm-v9",
            nombre="Factor transporte X",
        ...
        VersionFactorAmbiental.objects.create(
            ...
            version=1,
            valor=Decimal("0.1"),
            fuente="Fuente tecnica de prueba",
            estado="activo",
        )
        method = MetodologiaAmbiental.objects.create(
            ...
            categoria="transporte",
            flujo="transporte_tkm",
        )
        version = VersionMetodologia.objects.create(
            metodologia=method, version=1, estado="borrador"
        )
        formula = FormulaAmbiental.objects.create(
            version_metodologia=version,
            factor_ambiental=factor,
            codigo="formula-tkm-v9",
            tipo="transporte_tkm",
            expresion_legible="masa x distancia x factor",
        )
        VariableFormula.objects.create(
            formula=formula,
            clave="distancia",
            concepto_observacion="distancia_recorrida_km",
            unidad_esperada="km",
        )
        VariableFormula.objects.create(
            formula=formula,
            clave="masa",
            concepto_observacion="masa_transportada_t",
            unidad_esperada="t",
        )
        ...
        self.problem = ProblematicaAmbiental.objects.create(
            ...
            descripcion="Intensidad elevada",
            categoria="transporte",
        ...
        self.indicator = IndicadorAmbiental.objects.create(
            organizacion=self.org,
            codigo="intensidad-v9",
            nombre="Intensidad kgCO2e/t",
            tipo="intensidad",
            unidad="kgCO2e/t",
        ...
        AlcanceProblematica.objects.create(
            problematica=self.problem,
            actividad_operacional=self.activity,
            indicador=self.indicator,
        )
        ...
        self.action = AccionMejoraAmbiental.objects.create(
            problematica=self.problem,
            titulo="Consolidar cargas",
            descripcion="Consolidacion",
        )
        select_action(self.action)
        start_action(self.action, confirmed=True)
        ValorIndicador.objects.create(
            indicador=self.indicator,
    ...
    def review(self, state=None):
        row = RevisionProfesionalAmbiental.objects.create(
            organizacion=self.org, tipo="problematica", problematica=self.problem
        )
        if state:
            decide_review(
                row,
                ...
                ["respaldo adicional"] if state == "solicita_antecedentes" else [],
                self.professional,
            )
        return row
    ...
    def other_case(self):
        problem = ProblematicaAmbiental.objects.create(
            organizacion=self.org,
            titulo="Consumo energetico anomalo",
            descripcion="Caso B",
            categoria="energia",
            valor_inicial=5,
            objetivo_meta=4,
        ...
        action = AccionMejoraAmbiental.objects.create(
            problematica=problem, titulo="Ajustar consumo", descripcion="Caso B"
        )
        ...
        result = SnapshotIntervencion.objects.create(
            problematica=problem,
            accion=action,
            ciclo=1,
            tipo="resultado",
        ...
        intervention = self.result.__class__.objects.create(
            problematica=problem,
            accion=action,
            ciclo=1,
            snapshot_base=base,
            snapshot_resultado=result,
            estado="inconclusa",
    ...
    def test_revision_tipo_debe_corresponder_a_referencia(self):
        valid = RevisionProfesionalAmbiental.objects.create(
            organizacion=self.org, tipo="calculo", calculo=self.calculation
        )
        self.assertEqual(valid.calculo_id, self.calculation.id)
        evidence = EvidenciaObra.objects.create(
            organizacion=self.org,
            nombre="Respaldo",
            archivo=SimpleUploadedFile("respaldo.txt", b"dato"),
        )
        self.assertEqual(
            RevisionProfesionalAmbiental.objects.create(
                organizacion=self.org, tipo="evidencia", evidencia=evidence
            ).evidencia_id,
            evidence.id,
        )
        with self.assertRaises(ValidationError):
            RevisionProfesionalAmbiental.objects.create(
                organizacion=self.org, tipo="calculo", evidencia=evidence
            )
        with self.assertRaises(ValidationError):
            RevisionProfesionalAmbiental.objects.create(
                organizacion=self.org, tipo="calculo"
            )
        with self.assertRaises(ValidationError):
            RevisionProfesionalAmbiental.objects.create(
                organizacion=self.other, tipo="calculo", calculo=self.calculation
            )
    ...
    def test_servicio_informe_exige_tipo_y_objeto_principal(self):
        ...
        with self.assertRaises(ValidationError):
            generate_report(
                self.org, "desconocido", self.professional, problem=self.problem
            )

    def test_validacion_exige_version_generada_completa(self):
        draft = InformeAmbiental.objects.create(
            organizacion=self.org,
            tipo="problematica",
            problematica=self.problem,
            version=99,
            estado="borrador",
            generado_por=self.professional,
        )
        with self.assertRaises(ValidationError):
            validate_report(draft, self.professional)
        report = generate_report(
            self.org, "problematica", self.professional, problem=self.problem
        )
    ...
    def test_crear_revision_y_rechazar_cross_tenant(self):
        response = self.client.post(
            f"{self.base}/revisiones-profesionales/",
            {"tipo": "problematica", "problematica": self.problem.id},
            format="json",
        )
        ...
        self.assertEqual(
            self.client.post(
                f"{self.base}/revisiones-profesionales/",
                {"tipo": "problematica", "problematica": foreign.id},
                format="json",
            ).status_code,
            400,
        )
    ...
    def test_hallazgo_validacion_y_revision_decidida_inmutable(self):
        ...
        finding = self.client.post(
            f"{self.base}/revisiones-profesionales/{review.id}/hallazgos/",
            {
                "tipo": "observacion",
                "severidad": "media",
                "observacion": "Trazabilidad completa",
                "recomendacion": "Conservar anexos",
            },
            format="json",
        )
        self.assertEqual(finding.status_code, 201)
        decision = self.client.post(
            f"{self.base}/revisiones-profesionales/{review.id}/decision/",
            {
                "estado": "validada_con_observaciones",
                "conclusion": "Validada",
                "observaciones": "Mantener referencias",
            },
            format="json",
        )
        self.assertEqual(decision.status_code, 200)
        review.refresh_from_db()
        self.assertEqual(review.profesional, self.professional)
        review.observaciones = "Cambio destructivo"
        with self.assertRaises(ValidationError):
            review.save()

    def test_solicitar_antecedentes_y_usuario_sin_permiso_no_valida(self):
        requested = self.review("solicita_antecedentes")
        self.assertEqual(requested.antecedentes_solicitados, ["respaldo adicional"])
        pending = self.review()
        self.client.force_login(self.operator)
        response = self.client.post(
            f"{self.base}/revisiones-profesionales/{pending.id}/decision/",
    ...
    def test_correccion_no_cambia_observacion_original(self):
        ...
        correction = create_correction(
            review,
            self.professional,
            "Valor requiere aclaracion",
            {"valor": "132"},
            {"valor": "133"},
            observacion_afectada=self.distance,
        )
        self.distance.refresh_from_db()
        self.assertEqual(self.distance.valor_numerico, Decimal("132"))
        self.assertEqual(correction.propuesta_corregida["valor"], "133")
    ...
    def test_correccion_cross_tenant_rechazada_por_servicio_y_modelo(self):
        ...
        foreign_calculation = CalculoAmbiental.objects.create(
            ...
            formula=self.calculation.formula,
            version_factor=self.calculation.version_factor,
            resultado=1,
            unidad_resultado="kgCO2e",
        ...
        with self.assertRaises(ValidationError):
            create_correction(
                ...
                {},
                {},
                calculo_afectado=foreign_calculation,
            )
        with self.assertRaises(ValidationError):
            CorreccionHistoricaAmbiental.objects.create(
                organizacion=self.org,
                revision_origen=review,
                autor=self.professional,
                motivo="Creacion directa ajena",
                valor_estado_anterior={},
                propuesta_corregida={},
                calculo_afectado=foreign_calculation,
            )
        foreign_problem = ProblematicaAmbiental.objects.create(
            organizacion=self.other,
            titulo="Caso extranjero",
            descripcion="B",
            categoria="energia",
            valor_inicial=2,
            objetivo_meta=1,
        ...
        foreign_action = AccionMejoraAmbiental.objects.create(
            problematica=foreign_problem, titulo="Accion B", descripcion="B"
        )
        foreign_snapshot = SnapshotIntervencion.objects.create(
            problematica=foreign_problem,
            accion=foreign_action,
            ciclo=1,
            tipo="base",
            fecha=date.today(),
            congelado=True,
        ...
        with self.assertRaises(ValidationError):
            CorreccionHistoricaAmbiental.objects.create(
                ...
                revision_origen=review,
                autor=self.professional,
                motivo="Snapshot ajeno",
                valor_estado_anterior={},
                propuesta_corregida={},
                snapshot_afectado=foreign_snapshot,
            )
    ...
    def test_recalculo_por_correccion_crea_nuevo_calculo(self):
        ...
        correction = create_correction(
            review,
            self.professional,
            "Recalcular sin sobrescribir",
            {"calculo": self.calculation.id},
            {"recalcular": True},
            calculo_afectado=self.calculation,
        )
        new = recalculate_for_correction(correction)
        self.calculation.refresh_from_db()
        self.assertNotEqual(new.id, self.calculation.id)
        self.assertEqual(
            CalculoAmbiental.objects.filter(actividad=self.activity).count(), 2
        )
    ...
    def test_eventos_auditoria_y_expediente_referencias(self):
        ...
        self.assertIn(self.action.id, dossier.referencias["acciones"])
        self.assertIn(self.result.id, dossier.referencias["resultados"])
        self.assertTrue(
            EventoAuditoriaAmbiental.objects.filter(
                tipo="creacion_expediente", referencia=str(dossier.id)
            ).exists()
        )
    ...
    def test_cerrar_y_reabrir_conserva_cierre(self):
        ...
        closed = self.client.post(
            f"{self.base}/expedientes/{dossier.id}/cerrar/", {}, format="json"
        )
        self.assertEqual(closed.status_code, 200)
        closed_at = closed.data["cerrado_at"]
        reopened = self.client.post(
            f"{self.base}/expedientes/{dossier.id}/reabrir/",
            {"motivo": "Nueva evidencia"},
            format="json",
        )
        self.assertEqual(reopened.status_code, 200)
        self.assertEqual(reopened.data["cerrado_at"], closed_at)
    ...
    def test_caso_obligatorio_informe_pdf_snapshot_checksum_y_trazabilidad(self):
        ...
        with report.archivo.open("rb") as stream:
            content = stream.read()
            self.assertEqual(
                hashlib.sha256(content).hexdigest(), report.checksum_sha256
            )
        ...
        self.assertEqual(
            report.snapshot.contenido["problematica"]["titulo"],
            "Alta intensidad transporte",
        )
    ...
    def test_snapshot_estable_regenerar_crea_version_y_checksum_nuevo(self):
        ...
        frozen = first.snapshot.contenido.copy()
        self.problem.titulo = "Titulo posterior"
        self.problem.save()
        first.snapshot.refresh_from_db()
        self.assertEqual(first.snapshot.contenido, frozen)
    ...
    def test_ia_no_equivale_revision_y_negativos_aparecen(self):
        proposal = RecomendacionAgenteAmbiental.objects.create(
            ...
            accion="Propuesta",
            justificacion="IA",
            indicador_afectado="intensidad-v9",
            resultado_esperado="Pendiente",
            prioridad="media",
            periodo_seguimiento="30",
        ...
        HitoDecisionIA.objects.create(
            ...
            propuesta=proposal,
            tipo="propuesta",
            resumen="Propuesta IA",
        )
        self.assertEqual(self.problem.revisiones_profesionales.count(), 0)
        self.result.estado = "negativa"
        self.result.save(update_fields=["estado"])
        report = generate_report(
            self.org, "problematica", self.professional, problem=self.problem
        )
        self.assertEqual(
            report.snapshot.contenido["problematica"]["resultados_negativos"][0]["id"],
            self.result.id,
        )

    def test_api_informe_pdf_y_tenant(self):
        created = self.client.post(
            f"{self.base}/informes/",
            {"tipo": "problematica", "problematica": self.problem.id},
            format="json",
        )
        ...
        self.assertEqual(
            self.client.get(
                f"{self.base}/informes/{created.data['id']}/pdf/"
            ).status_code,
            200,
        )
        foreign = ProblematicaAmbiental.objects.create(
    ...
    def test_expedientes_filtran_por_obra(
        self,
    ):
        work_a = Obra.objects.create(
            organizacion=self.org,
            nombre="Obra Profesional A",
            fecha_inicio=date(
                2026,
                1,
                1,
        ...
        problem_b = ProblematicaAmbiental.objects.create(
            organizacion=self.org,
            obra=work_b,
            titulo="Problema obra B",
            descripcion="Caso de otra obra",
            categoria="agua",
            indicador="consumo_agua",
            unidad_indicador="m3",
            valor_inicial=10,
            objetivo_meta=8,
        ...
        dossier_a = create_dossier(
            self.problem,
            self.professional,
        )

        dossier_b = create_dossier(
            problem_b,
            self.professional,
        )

        response = self.client.get(f"{self.base}/expedientes/" f"?obra={work_a.id}")
    ...
    def test_expediente_de_otra_obra_no_es_visible(
        self,
    ):
        work_a = Obra.objects.create(
            organizacion=self.org,
        ...
        response = self.client.get(
            f"{self.base}/expedientes/" f"{dossier.id}/" f"?obra={work_a.id}"
        )
