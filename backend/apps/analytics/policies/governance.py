...
def structural_errors(version):
    ...
    dynamic_factor_formula = (
        dynamic_fuel_formula or formula.tipo == FormulaAmbiental.Tipo.MATERIAL_CANTIDAD
    )
    ...
    governed_energy_shell = (
        formula.tipo == FormulaAmbiental.Tipo.ENERGIA_CONSUMIDA
        and formula.factor_ambiental_id is not None
        and formula.factor_ambiental.organizacion_id is None
    )
    if (
        not dynamic_factor_formula
        and not governed_energy_shell
        and not active_factor_version(formula, version.metodologia.organizacion)
    ):
