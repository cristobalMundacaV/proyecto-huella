...
from .selectors.knowledge import (
    intervention_for_organization,
    knowledge_case_for_organization,
    knowledge_cases_for_organization,
)
...
def knowledge_cases(request, organizacion_id):
    ...
    if request.method == "GET":
        rows = knowledge_cases_for_organization(organization)
        return Response(CasoConocimientoPrivadoSerializer(rows, many=True).data)
    result = get_object_or_404(
        intervention_for_organization(organization, request.data.get("intervencion"))
    )
    try:
    ...
    return Response(
        CasoConocimientoPrivadoSerializer(case).data, status=201 if created else 200
    )
...
def knowledge_case_detail(request, organizacion_id, case_id):
    ...
    if not organization:
        return Response({"detail": "Recurso no encontrado."}, status=404)
    case = get_object_or_404(knowledge_case_for_organization(organization, case_id))
