...
def simulate_rows(
    ...
    for row in rows:
        actividad = normalize_activity(row.get("actividad"))
        ...
        if should_apply_company and actividad == "diesel":
            cantidad *= 1 - to_float(diesel_reduction) / 100

        if should_apply_company and actividad == "electricidad":
...
def optimize_rows(rows):
    ...
    for diesel_reduction in range(0, 81, 5):
        for electricity_increase in range(0, 61, 5):
...
def calculate_risk_profile(summary, optimized_scenario=None):
    ...
    diesel_present = any(normalize_activity(row.get("actividad")) == "diesel" for row in rows)
    potential_reduction = (
