...
const getInsightEmissionValue = (row = {}) =>
  Number(
...
const formatInsightPercent = (value) =>
  `${formatNumber(Number(value || 0), 1)}%`;

function getConstructionActionPack(category, source) {
  const normalizedCategory = normalizeInsightText(category);
  const normalizedSource = normalizeInsightText(source);

  if (normalizedCategory.includes("material")) {
    const isSteel = normalizedSource.includes("acero");
    const isConcrete =
      normalizedSource.includes("hormigon") || normalizedSource.includes("cemento");

    return {
      intervention: isSteel
        ? "Piloto de acero con menor carbono incorporado"
        : isConcrete
          ? "Piloto de hormigón/cemento con menor factor"
          : "Piloto de materiales de menor impacto",
      actions: [
        isSteel
          ? "Solicitar al proveedor ficha técnica, origen y factor kg CO2e/ton del acero utilizado."
          : "Solicitar ficha técnica, composición y factor kg CO2e por unidad del material crítico.",
        "Comparar el proveedor actual contra al menos dos alternativas con menor factor de emisión.",
        "Revisar cubicaciones, mermas y sobreconsumo en la etapa prioritaria antes de nuevas compras.",
        "Vincular facturas, guías de despacho y fichas técnicas al registro de emisión crítico.",
      ],
      metrics: [
        "kg CO2e por unidad comprada",
        "merma de material",
  ...
  if (normalizedCategory.includes("transporte")) {
    return {
      intervention: "Optimización logística de proveedores y despachos",
      actions: [
        "Identificar viajes con baja carga útil o recorridos repetidos hacia la obra.",
        "Consolidar despachos de materiales críticos y reducir viajes parciales.",
        "Priorizar proveedores cercanos para áridos, hormigón y materiales de alta rotación.",
        "Registrar distancia, patente, litros y guía de despacho en cada traslado relevante.",
      ],
      metrics: ["km por despacho", "litros por viaje", "tonelada-km"],
    };
  }

  if (normalizedCategory.includes("maquinaria")) {
    return {
      intervention: "Control operativo de maquinaria diésel",
      actions: [
        "Medir litros por equipo y por frente de trabajo.",
        "Reducir ralentí y tiempos muertos en excavadoras, retroexcavadoras y generadores.",
        "Ajustar la planificación diaria para evitar equipos encendidos sin producción.",
        "Registrar mantenciones y horas máquina como evidencia operacional.",
      ],
      metrics: ["litros por hora", "horas máquina", "tiempo en ralentí"],
    };
  }

  if (normalizedCategory.includes("energia")) {
    return {
      intervention: "Reducción de consumo energético de faena",
      actions: [
        "Separar consumo de red, generadores y equipos temporales.",
        "Reducir uso de generadores cuando exista alternativa de conexión eléctrica.",
        "Revisar horarios de mayor consumo y equipos con uso permanente.",
        "Vincular boletas eléctricas o registros de generador a la obra.",
      ],
      metrics: ["kWh semanal", "litros generador", "kg CO2e/kWh"],
    };
  }

  if (normalizedCategory.includes("residuo")) {
    return {
      intervention: "Segregación y valorización de residuos de obra",
      actions: [
        "Separar escombros, fierro, madera, plásticos y residuos mixtos.",
        "Priorizar retiro valorizado antes de disposición final.",
        "Exigir ticket de pesaje y certificado de retiro por gestor.",
        "Medir reducción de residuos mixtos por etapa de obra.",
      ],
      metrics: ["ton residuos mixtos", "% valorizado", "tickets validados"],
    };
  }

  return {
    intervention: "Ordenamiento de datos y priorización de fuente crítica",
    actions: [
      "Clasificar correctamente la fuente de emisión crítica.",
      "Validar cantidad, unidad y factor de emisión utilizado.",
      "Asociar el registro a una etapa y obra específica.",
      "Subir evidencia documental para respaldar el cálculo.",
    ],
    metrics: ["registros completos", "factores validados", "evidencias"],
  };
}

function InsightPanel({ data, scenario, riskProfile, environmentalStatus }) {
  const rows = Array.isArray(data?.datos) ? data.datos : [];
  const total = Number(data?.total_emisiones || 0);

  const topSourcesFromRows = Object.values(
    rows.reduce((accumulator, row) => {
      const source = row.fuente_emision || "Sin fuente";
      const category = row.categoria_visible || row.categoria || "Otros";
      const stage = row.etapa_nombre || row.etapa || "Sin etapa";
      ...
      current.emissions += getInsightEmissionValue(row);
      current.records += 1;
  ...
  const topSource = topSourcesFromRows[0] || null;

  const source =
    data?.fuente_critica ||
    topSource?.source ||
    "Fuente crítica sin datos";

  const category =
    data?.categoria_critica ||
    topSource?.category ||
    "Otros";
  ...
  const sourceEmissions = Number(topSource?.emissions || 0);
  const sourceShare =
    total > 0 && sourceEmissions > 0
      ? (sourceEmissions / total) * 100
      : Number(riskProfile?.factors?.sourceConcentration || 0);

  const reductionPct = Number(
    scenario?.reductionPct ||
      riskProfile?.factors?.potentialReduction ||
      0
  );
