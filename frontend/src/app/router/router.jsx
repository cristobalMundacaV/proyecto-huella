...
function LoginRoute() { const { loadingAuth, user } = useAuth(); const location = useLocation(); const organizationState = useOrganizacionActiva(); if (loadingAuth) return <PlatformLoader fullScreen title="Iniciando sesión" />; if (!user) return <LoginPage />; if (user.is_superuser) return <Navigate to="/saas" replace />; const access = resolveOrganizationAccess({ resolving: organizationState.resolvingOrganizations, error: organizationState.errorOrganizaciones, organizations: organizationState.organizaciones, activeOrganization: organizationState.activeOrganizacion }); if (access === "resolving") return <PlatformLoader fullScreen title="Preparando tu organización" />; if (access === "error") return <ErrorState title="No pudimos cargar tu organización" description={organizationState.errorOrganizaciones} onRetry={() => organizationState.refreshOrganizaciones()} />; if (access === "no-organization") return <NoOrganizationState />; if (access === "selection-required") return <Navigate to="/seleccionar-organizacion" replace />; return <Navigate to={organizationDestination(organizationState.activeOrganizacion, location.state?.returnTo || "/inicio")} replace />; }
function RequireOrganization() { const organizationState = useOrganizacionActiva(); const access = resolveOrganizationAccess({ resolving: organizationState.resolvingOrganizations, error: organizationState.errorOrganizaciones, organizations: organizationState.organizaciones, activeOrganization: organizationState.activeOrganizacion }); if (access === "resolving") return <PlatformLoader fullScreen title="Cargando empresas" />; if (access === "error") return <ErrorState title="No pudimos cargar tu organización" description={organizationState.errorOrganizaciones} onRetry={() => organizationState.refreshOrganizaciones()} />; if (access === "no-organization") return <NoOrganizationState />; return access === "selection-required" ? <Navigate to="/seleccionar-organizacion" replace /> : <Outlet />; }
