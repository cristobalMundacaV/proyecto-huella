  ...
  return (
    <section className={`rounded-3xl border p-4 sm:p-6 shadow-xl ${riskFrameClass}`}>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 lg:grid-cols-3">
        {strategicPlan.actionLevels.map((level) => (
          <div key={level.label} className={`rounded-2xl border p-4 ${level.tone}`}>
            ...
            <p className="mt-1 text-lg font-bold">{level.range}</p>
      ...
      <div className="mt-5 rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 text-sm text-slate-300 sm:grid-cols-2 xl:grid-cols-5">
          <span>
            Emisiones totales:{" "}
...
function SummaryItem({ label, value }) {
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
      <p className="text-xs text-slate-500">{formatTitleCase(label)}</p>
