...
function FactoresPage() {
  ...
  if (!activeConstructoraId) {
    return (
      <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-8 text-center text-[var(--text-muted)]">
        Selecciona una empresa para gestionar factores de emision.
      </div>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8">
      ...
      {loading && <p className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-4 text-sm font-semibold text-[var(--text-muted)]">Cargando factores...</p>}
      {message && <p className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-800">{message}</p>}
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_0.8fr]">
        <FactorCatalog config={config} factors={factors} onCreate={() => setCreateOpen(true)} />
        <div className="space-y-4">
          <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-[var(--shadow-premium)]">
            ...
            <div className="mt-4 space-y-3">
              {recommendations.map((item) => <p key={item} className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">{item}</p>)}
            </div>
          </div>
          <FactorSuggestionPanel suggestion={firstSuggestion} />
        </div>
      </section>
      <PendingFactorRecords factors={factors} pendingRecords={pendingRecords} suggestFactor={config.suggestionRules.suggestFactor} onApply={(record, suggestion) => setApplyState({ record, suggestion })} />
      {createOpen && <FactorCreateModal config={config} onClose={() => setCreateOpen(false)} onSubmit={handleCreateFactor} preset={activePreset.key} />}
      {applyState && <FactorApplyModal factors={factors} onApply={handleApplyFactor} onClose={() => setApplyState(null)} record={applyState.record} suggestion={applyState.suggestion} />}
    </main>
  );
}
