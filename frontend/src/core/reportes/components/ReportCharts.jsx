...
function ReportCharts({ report, reportConfig }) {
  ...
  return (
    <section className="grid gap-6 lg:grid-cols-2">
      <ChartCard title="Serie temporal" description="Evolucion de emisiones segun la agrupacion seleccionada.">
        {report.serie.length > 1 ? (
          ...
          <EmptyChart text="Se necesita mas de un periodo para visualizar tendencia temporal." />
        )}
      </ChartCard>

      <ChartCard title="Distribucion por categoria" description="Comparativo por categoria del preset activo.">
        {categoryData.length ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={categoryData} margin={{ top: 10, right: 10, left: 0, bottom: 24 }}>
              ...
              <YAxis tick={{ fill: "#475569", fontSize: 12, fontWeight: 600 }} tickFormatter={(value) => formatReportNumber(value)} width={72} />
              <Tooltip cursor={false} content={<ReportTooltip labelPrefix="Categoria" />} />
          ...
          <EmptyChart text="No hay categorias con emisiones en este periodo." />
        )}
      </ChartCard>

      <ChartCard title="Distribucion por modulo operativo" description="Agrupacion por etapa, modulo o proceso segun preset.">
        {report.modules.length ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={report.modules.slice(0, 8)} layout="vertical" margin={{ top: 8, right: 16, left: 28, bottom: 8 }}>
              ...
              <Tooltip cursor={false} content={<ReportTooltip labelPrefix="Modulo" />} />
              <Bar dataKey="emisiones" radius={[0, 8, 8, 0]} fill="#0F766E" />
            </BarChart>
          </ResponsiveContainer>
        ) : (
          <EmptyChart text="No hay modulos operativos para visualizar." />
        )}
      </ChartCard>

      <ChartCard title="Top fuentes criticas" description="Fuentes de emision con mayor impacto del periodo.">
        {report.fuentes.length ? (
          <div className="space-y-3">
            {report.fuentes.slice(0, 5).map((item) => (
          ...
          <EmptyChart text="No hay fuentes criticas en este periodo." />
        )}
      </ChartCard>
    </section>
  );
