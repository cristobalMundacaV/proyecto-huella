...
import { createTraceableAction, getTraceableActions, getTraceableActionsSummary } from "@/features/intelligence/services/traceableActionsApi";
...
function buildDecisionAgenda({ actionsSummary, criticalSource, records }) {
  ...
  if (records) {
    agenda.push({ priority: "Alta", decision: `Priorizar foco crítico: ${criticalSource}`, reason: "Es el principal punto de concentración de huella del periodo analizado.", expected: "Reducir impacto donde existe mayor potencial de mejora." });
  } else {
...
function CycleMetric({ label, value }) {
  return <div className="rounded-2xl border border-cyan-200 bg-white/85 px-4 py-3 text-center"><p className="text-[10px] font-black uppercase tracking-wide text-cyan-700">{label}</p><p className="mt-1 text-2xl font-black text-cyan-950">{value}</p></div>;
}
