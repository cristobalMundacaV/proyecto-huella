    ...
    : [
        {
          key: "sin_brechas",
          level: "Baja",
          title: "Sin brechas críticas",
          description: "El reporte no muestra brechas relevantes para presentación ejecutiva.",
          action: "Mantener seguimiento y actualizar datos del próximo periodo.",
        },
      ];
}
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 xl:grid-cols-2">
        {risks.map((risk, index) => (
          <article key={`${risk.key}-${index}`} className="rounded-3xl border border-rose-100 bg-white p-4 shadow-[0_10px_22px_rgba(15,23,42,0.04)]">
            ...
            {risk.key !== "sin_brechas" ? (
              <button
                type="button"
                disabled={creatingRiskKey === risk.key}
                onClick={() => onCreateAction?.(risk)}
                className="mt-3 inline-flex items-center justify-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-2 text-xs font-black text-rose-800 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <CheckCircle2 size={15} />
                {creatingRiskKey === risk.key ? "Creando acción..." : "Crear acción desde brecha"}
              </button>
            ) : null}
          </article>
        ))}
        ...
        <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
          ...
          <div className="mt-3 space-y-2">
            {brief.bullets.map((bullet) => (
              <div key={bullet} className="rounded-2xl border border-slate-100 bg-white px-3 py-2 text-sm font-bold text-slate-700">{bullet}</div>
            ))}
          </div>
        </div>
      </div>
          ...
          agenda.map((item, index) => {
            ...
            return (
              <article key={`${item.decision}-${index}`} className="rounded-3xl border border-amber-100 bg-white p-4 shadow-[0_10px_22px_rgba(15,23,42,0.04)]">
                ...
                <button
                  type="button"
                  disabled={creatingDecisionKey === decisionKey}
                  onClick={() => onCreateAction?.(item, index)}
                  className="mt-3 inline-flex items-center justify-center gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-black text-amber-800 hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <CheckCircle2 size={15} />
                  {creatingDecisionKey === decisionKey ? "Creando acción..." : "Crear acción desde decisión"}
                </button>
              </article>
            );
          })
  ...
  return (
    <section className="rounded-3xl border border-[var(--border)] bg-[linear-gradient(135deg,#FFFFFF_0%,#F8FAFC_45%,#ECFDF5_100%)] p-8 text-center shadow-[var(--shadow-card)]">
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        <button onClick={onImport} className="inline-flex items-center gap-2 rounded-2xl border border-[#A7F3D0] bg-[#ECFDF5] px-5 py-3 text-sm font-black text-[#047857] shadow-[0_12px_24px_rgba(15,23,42,0.06)]"><Database size={17} />Ir a Importacion de datos</button>
