...
function buildReportReadiness({ actionsSummary, records, totalEmissions }) {
  ...
  const checks = [
    {
      key: "records",
      label: "Datos ambientales cargados",
      passed: records > 0,
      detail: records > 0 ? `${records} registros analizados` : "Faltan registros ambientales para sustentar el reporte.",
    },
    {
      key: "footprint",
      label: "Huella calculada",
      passed: totalEmissions !== "Sin datos",
      detail: totalEmissions !== "Sin datos" ? `Huella disponible: ${totalEmissions}` : "No hay huella total calculada para el periodo.",
    },
    {
      key: "actions",
      label: "Acciones ambientales registradas",
      passed: actionsTotal > 0,
      detail: actionsTotal > 0 ? `${actionsTotal} acciones disponibles para seguimiento` : "Falta crear acciones para cerrar el ciclo de gestión.",
    },
    {
      key: "traceability",
      label: "Trazabilidad operacional suficiente",
      passed: actionsTotal > 0 && traceabilityPct >= 80,
      detail: actionsTotal > 0 ? `${traceabilityPct}% de acciones con vínculo` : "Sin acciones para evaluar trazabilidad.",
    },
    {
      key: "overdue",
      label: "Sin acciones vencidas críticas",
      passed: overdue === 0,
      detail: overdue === 0 ? "No hay acciones vencidas" : `${overdue} acciones vencidas requieren revisión`,
    },
    {
      key: "closure",
      label: "Avance de cierre visible",
      passed: actionsTotal === 0 ? false : completionPct >= 30,
      detail: actionsTotal > 0 ? `${completionPct}% de cierre` : "Sin acciones cerradas para demostrar avance.",
    },
  ];
  ...
  if (!activeConstructoraId) {
    return (
      <main className="mx-auto max-w-7xl text-[var(--text-main)]">
        ...
        <div className="mt-8 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-8 text-center text-[var(--text-muted)]">Selecciona o crea una empresa para revisar reportes temporales.</div>
      </main>
    );
  }
...
function ReportReadinessCard({ readiness }) {
  ...
  return (
    <section className="rounded-3xl border border-sky-200 bg-sky-50/70 p-5 shadow-[var(--shadow-card)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-sky-700">Estado de preparación</p>
          <h2 className="mt-1 text-2xl font-black text-[var(--text-main)]">{readiness.status}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-sky-900">El reporte cumple {readiness.passed} de {readiness.total} criterios mínimos para presentación ejecutiva.</p>
        </div>
        <div className="rounded-3xl border border-sky-200 bg-white px-5 py-4 text-center shadow-sm">
          <p className="text-[10px] font-black uppercase tracking-wide text-sky-700">Preparación</p>
          <p className="mt-1 text-3xl font-black text-sky-950">{readiness.score}%</p>
        </div>
      </div>
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[var(--shadow-card)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-center"><p className="text-[10px] font-black uppercase tracking-wide text-emerald-700">Salida</p><p className="mt-1 text-sm font-black text-emerald-950">TXT + JSON</p></div>
      </div>
      <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
          ...
          <div className="mt-3 space-y-2">{brief.bullets.map((bullet) => <div key={bullet} className="rounded-2xl border border-slate-100 bg-white px-3 py-2 text-sm font-bold text-slate-700">{bullet}</div>)}</div>
        </div>
      </div>
    </section>
  );
...
function DecisionAgendaCard({ agenda = [] }) {
  return (
    <section className="rounded-3xl border border-amber-200 bg-amber-50/70 p-5 shadow-[var(--shadow-card)]">
      <div><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Agenda de decisión ejecutiva</p><h2 className="mt-1 text-2xl font-black text-[var(--text-main)]">Decisiones sugeridas para gerencia</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-amber-900">Prioridades concretas para transformar la lectura ambiental en acuerdos, responsables y seguimiento.</p></div>
      <div className="mt-5 grid grid-cols-1 gap-3 xl:grid-cols-2">
        {agenda.length ? agenda.map((item, index) => <article key={`${item.decision}-${index}`} className="rounded-3xl border border-amber-100 bg-white p-4 shadow-[0_10px_22px_rgba(15,23,42,0.04)]"><div className="flex items-center justify-between gap-3"><span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-black text-amber-800">{item.priority}</span><span className="text-xs font-black uppercase tracking-wide text-slate-400">Decisión {index + 1}</span></div><h3 className="mt-3 text-base font-black text-[var(--text-main)]">{item.decision}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{item.reason}</p><p className="mt-3 rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600"><strong>Resultado esperado:</strong> {item.expected}</p></article>) : <div className="rounded-3xl border border-dashed border-amber-200 bg-white/70 p-6 text-center text-sm font-bold text-amber-900">Sin decisiones sugeridas para este periodo.</div>}
      </div>
    </section>
  );
}
...
function ReportActionPlan({ actions = [], onOpenActions }) {
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[var(--shadow-card)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Plan de acción ambiental</p><h2 className="mt-1 text-2xl font-black text-[var(--text-main)]">Acciones priorizadas para seguimiento</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-muted)]">Lista de acciones recientes para revisar responsables, vencimientos y trazabilidad operacional desde el reporte.</p></div><button type="button" onClick={onOpenActions} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800 shadow-sm hover:bg-emerald-100"><CheckCircle2 size={17} />Abrir tablero</button></div>
      {actions.length ? <div className="mt-5 space-y-3">{actions.map((action, index) => <article key={action.id || `${action.title}-${index}`} className="grid gap-3 rounded-3xl border border-slate-100 bg-slate-50 p-4 lg:grid-cols-[1fr_170px_170px] lg:items-center"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-700">Acción {index + 1}</p><h3 className="mt-1 text-base font-black text-[var(--text-main)]">{action.title || "Acción ambiental"}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{action.description || "Sin descripción registrada."}</p><p className="mt-2 text-xs font-bold text-slate-500">{actionLinkLabel(action)}</p></div><div className="rounded-2xl border border-slate-100 bg-white px-3 py-2 text-sm"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">Responsable</p><p className="mt-1 font-black text-slate-800">{action.responsible || "Equipo ambiental"}</p></div><div className="rounded-2xl border border-slate-100 bg-white px-3 py-2 text-sm"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">Estado / fecha</p><p className="mt-1 font-black text-slate-800">{statusLabel(action.status)}</p><p className="text-xs font-bold text-slate-500">{action.dueDate || "Sin fecha"}</p></div></article>)}</div> : <div className="mt-5 rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm font-bold text-[var(--text-muted)]">No hay acciones recientes para listar en este reporte.</div>}
    </section>
  );
}
...
function ReportActionsSummary({ onOpenActions, summary }) {
  ...
  return (
    <section className="rounded-3xl border border-emerald-200 bg-emerald-50/60 p-5 shadow-[var(--shadow-card)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Gestión accionable</p><h2 className="mt-1 text-2xl font-black text-[var(--text-main)]">Acciones ambientales del periodo</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-emerald-900">{summary.total ? `${summary.active} acciones siguen activas, ${summary.completed} están cerradas y ${summary.traceabilityPct || 0}% tiene vínculo operacional.` : "Aún no hay acciones ambientales trazables para incorporar al reporte."}</p></div><button type="button" onClick={onOpenActions} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-white px-4 py-3 text-sm font-black text-emerald-800 shadow-sm hover:bg-emerald-50"><CheckCircle2 size={17} />Revisar acciones</button></div>
      <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-5"><ReportActionMetric icon={<CheckCircle2 size={16} />} label="Total" value={summary.total || 0} /><ReportActionMetric icon={<Clock3 size={16} />} label="Activas" value={summary.active || 0} /><ReportActionMetric icon={<Clock3 size={16} />} label="Vencidas" value={summary.overdue || 0} /><ReportActionMetric icon={<CheckCircle2 size={16} />} label="Cierre" value={`${summary.completionPct || 0}%`} /><ReportActionMetric icon={<CheckCircle2 size={16} />} label="Trazabilidad" value={`${summary.traceabilityPct || 0}%`} /></div>
    </section>
  );
}
