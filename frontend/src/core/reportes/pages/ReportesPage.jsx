...
function buildDecisionActionPayload(item, index) {
  const isHighPriority = item.priority === "Alta";
  return {
    title: `Ejecutar decisión: ${item.decision}`,
    description: `${item.reason}\n\nResultado esperado: ${item.expected}`,
    responsible: "Equipo ambiental",
    dueDate: todayPlus(isHighPriority ? 7 : 14),
    status: "pendiente",
    source: "Reporte ejecutivo · Agenda de decisión",
...
function buildDecisionAgenda({ actionsSummary, criticalSource, records }) {
  ...
  const agenda = [];
  if (records) agenda.push({ priority: "Alta", decision: `Priorizar foco crítico: ${criticalSource}`, reason: "Es el principal punto de concentración de huella del periodo analizado.", expected: "Reducir impacto donde existe mayor potencial de mejora." });
  else agenda.push({ priority: "Alta", decision: "Completar base de datos ambiental del periodo", reason: "Sin registros suficientes no existe diagnóstico confiable para gerencia.", expected: "Activar medición, comparación y trazabilidad ejecutiva." });
...
function buildImprovementCycle(actions = []) {
  const reportActions = actions.filter((action) => actionOrigin(action).fromReport);
  const riskActions = reportActions.filter((action) => actionOrigin(action).key === "risk");
  const decisionActions = reportActions.filter((action) => actionOrigin(action).key === "decision");
  const completed = reportActions.filter((action) => action.status === "completada");
  const open = reportActions.filter((action) => action.status !== "completada");
...
function ReportesPage({ activeOrganizacion: propActiveOrganizacion, activeOrganizacionId: propActiveOrganizacionId, onSetActiveView }) {
  ...
  const [traceableActions, setTraceableActions] = useState([]);
  ...
  const [creatingDecisionKey, setCreatingDecisionKey] = useState("");
  const [updatingPriorityActionId, setUpdatingPriorityActionId] = useState("");
  ...
  const loadReport = async (showLoading = true) => {
    ...
    try {
      ...
      if (actionsSummaryResult.status === "fulfilled") setActionsSummary(actionsSummaryResult.value); else setActionsSummary(null);
      if (actionsResult.status === "fulfilled") setTraceableActions(Array.isArray(actionsResult.value) ? actionsResult.value : []); else setTraceableActions([]);
  ...
  async function handleCreateRiskAction(risk) {
    ...
    try {
      ...
      setActionFeedback(requestError.response?.data?.error || "No se pudo crear la acción desde la brecha.");
    } finally {
      setCreatingRiskKey("");
    }
  }
  ...
  async function handleCreateDecisionAction(item, index) {
    ...
    try {
      ...
      setActionFeedback(requestError.response?.data?.error || "No se pudo crear la acción desde la decisión.");
    } finally {
      setCreatingDecisionKey("");
    }
  }
  ...
  async function handleUpdatePriorityActionStatus(action, nextStatus) {
    ...
    try {
      ...
      setActionFeedback(requestError.response?.data?.error || "No se pudo actualizar el estado de la acción.");
    } finally {
      setUpdatingPriorityActionId("");
    }
  }

  function openFiltersModal() { setDraftFilters(filters); setIsFiltersModalOpen(true); }
...
function ClientSummaryCard({ summary }) {
  ...
  async function handleCopy() { await navigator.clipboard?.writeText?.(summary.text); setCopied(true); window.setTimeout(() => setCopied(false), 1800); }
