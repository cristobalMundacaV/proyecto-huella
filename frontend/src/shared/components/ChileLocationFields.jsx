...
import { CHILE_REGIONS, getComunasByRegion } from "@/features/organizaciones/utils/chileRegions";
...
export default function ChileLocationFields({ region, comuna, onChange, errors = {}, onBlur, required = false }) {
  ...
  return <>
    <Select label="Región" required={required} value={region || ""} onChange={(event) => onChange({ region: event.target.value, comuna: "" })} onBlur={() => onBlur?.("region")} error={errors.region}>
