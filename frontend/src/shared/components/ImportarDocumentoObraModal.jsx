...
function toFileName(value) {
  return String(value || "documento").split(/[\\/]/).pop() || "documento";
}
...
async function downloadFileAsFile(fileUrl, fileName) {
  ...
  if (!response.ok) {
    throw new Error("No se pudo descargar el documento para analizarlo.");
  }
...
function ImportarDocumentoObraModal({
  activeEmpresaId,
  archivoNombre = "",
  archivoUrl = "",
  documentoId = "",
  ...
  useEffect(() => {
    ...
    async function loadCatalogs() {
      try {
        ...
        const [lotesData, factorsData] = await Promise.all([getEmpresaLotes(activeEmpresaId), getFactoresEmision()]);
        ...
        setSelectedLoteId((current) => current || initialLoteId || (Array.isArray(lotesData) && lotesData[0]?.id_lote ? String(lotesData[0].id_lote) : ""));
      } catch (loadError) {
        if (!cancelled) {
  ...
  }, [activeEmpresaId, initialLoteId, open]);
  ...
  }, [archivoNombre, initialLoteId, open]);

  const selectedLote = useMemo(
    () => lotes.find((item) => String(item.id_lote) === String(selectedLoteId)) || null,
    [lotes, selectedLoteId]
  );

  const factorOptions = useMemo(() => analysis?.emission?.factorSuggestions || factors.slice(0, 3), [analysis, factors]);

  const isExistingSource = Boolean(documentoId || archivoUrl);
  ...
  function buildStructuredPayload(textResult, jsonResult) {
    ...
    const suggestion = inferDocumentImportSuggestion({
      text,
      structured,
      fileName: file?.name || archivoNombre || sourceLabel || toFileName(archivoUrl || documentoId || "documento"),
      factors,
    });
    ...
    setDraft({
      activity: suggestion.emission.activity || "",
    ...
    try {
      ...
      if (documentoId) {
        const [textResult, jsonResult] = await Promise.all([
          extractDocumentTextById(documentoId),
          extractDocumentJsonById(documentoId),
        ]);
      ...
      if (!sourceFile) {
        if (!file) {
          throw new Error("Carga un archivo o selecciona un documento existente para iniciar el analisis.");
        }
      ...
      setError(analysisError?.response?.data?.error || analysisError?.message || "No se pudo analizar el documento.");
    } finally {
      setLoadingAnalysis(false);
    }
  }
  ...
  useEffect(() => {
    if (!open || loadingCatalogs || analysis || (!documentoId && !archivoUrl)) {
      return;
    }

    runAnalysis();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, documentoId, archivoUrl, loadingCatalogs]);
  ...
  async function handleSaveEvidence() {
    try {
      ...
      if (!sourceFile) {
        throw new Error("Este documento no tiene archivo descargable para guardarlo como evidencia.");
      }

      if (!documentDate) {
        throw new Error("Indica la fecha del documento antes de guardar la evidencia.");
      }

      setSavingEvidence(true);
      setError("");
      await uploadLoteDocumento(selectedLoteId, {
        tipo_documento: documentType,
        fecha: documentDate,
        archivo: sourceFile,
      });

      onEvidenceSaved?.({ loteId: selectedLoteId, fileName: sourceFile.name, documentType, documentDate });
      onClose?.();
    } catch (saveError) {
  ...
  async function handleCreateEmission() {
    try {
      ...
      const payload = {
        actividad: draft.activity || draft.source || "Documento importado",
      ...
      const result = await createLoteActividad(selectedLoteId, payload);
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">
              Extrae texto del documento, sugiere la lectura de obra y confirma manualmente antes de guardar.
            </p>
          </div>
        </div>
        <button
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="space-y-4 rounded-3xl border border-[var(--border)] bg-[var(--bg-surface)] p-4 sm:p-5">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--text-muted)]">
              <span className="rounded-full border border-[var(--border)] bg-[var(--bg-card)] px-3 py-1 font-semibold text-[var(--text-main)]">
                {sourceLabel || (documentoId ? `Documento ${documentoId}` : archivoUrl ? "Evidencia vinculada" : "Archivo nuevo")}
              </span>
              <span className="rounded-full border border-[var(--border)] bg-[var(--bg-card)] px-3 py-1 font-semibold text-[var(--text-main)]">
              ...
              >
                ...
                {lotes.map((item) => (
                  <option key={item.id_lote} value={item.id_lote}>
                    {item.id_lote} - {item.empresa_aserradero || item.origen || "Obra"}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              onClick={runAnalysis}
              disabled={loadingAnalysis || (!file && !documentoId && !archivoUrl)}
          ...
          <div className="rounded-3xl border border-[#B9D8D3] bg-[var(--info-bg)] p-4">
            ...
            <p className="mt-2 text-xs text-[#075985]/80">
              Si el documento no se lee bien, puedes guardar la evidencia y completar el registro manualmente.
            </p>
          </div>
        </section>
        ...
        <section className="space-y-4 rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-4 sm:p-5">
          <div>
            ...
            <h3 className="mt-2 text-xl font-bold text-[var(--text-main)]">Resumen del documento</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="space-y-2 text-sm font-semibold text-[var(--text-main)]">
              <span>Fuente / actividad</span>
              <input
                value={draft.source}
                onChange={(event) => setDraft((current) => ({ ...current, source: event.target.value, activity: event.target.value }))}
                className="w-full rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)] outline-none"
              />
            </label>
            <label className="space-y-2 text-sm font-semibold text-[var(--text-main)]">
            ...
            >
              ...
              {factorOptions.map((item) => (
                <option key={item.id || item.factor_emision_id || item.label} value={item.id || item.factor_emision_id}>
                  {item.label || item.actividad} {item.unidad ? `| ${item.unidad}` : ""}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
            ...
            <p className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap text-sm leading-6 text-[var(--text-main)]">
              {analysis?.text || "Aun no se analizo el documento."}
            </p>
          </div>
          ...
          <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-surface)] p-4">
            ...
            <p className="mt-2 text-sm text-[var(--text-main)]">
              {selectedLote ? `${selectedLote.id_lote} · ${selectedLote.empresa_aserradero || selectedLote.origen || "Obra"}` : "Selecciona una obra destino para continuar."}
            </p>
...
export default ImportarDocumentoObraModal;
