...
export default function WorkCompliancePage() {
  ...
  return <main className="space-y-7">
    ...
    {noData ? <><div className="grid gap-4 md:grid-cols-3"><KpiCard label="Documentos válidos" value="Sin lectura" helper="Aún no hay documentos asociados" icon={FileText} /><KpiCard label="Alertas abiertas" value="Sin lectura" helper="No equivale a cero alertas" icon={AlertTriangle} /><KpiCard label="Estado general" value="Por configurar" helper="Requiere antecedentes verificables" icon={ShieldCheck} /></div><EmptyState icon={ShieldCheck} title="Cumplimiento preparado para recibir antecedentes" description="Esta obra todavía no tiene documentos ambientales asociados, por lo que no es posible determinar su estado de cumplimiento." guidance="Agrega el primer respaldo documental para comenzar a evaluar vigencias, validaciones y alertas." suggestions={["Permisos y resoluciones", "Certificados", "Registros de seguimiento"]} primaryAction={<Link className="inline-flex min-h-11 items-center rounded-xl bg-emerald-700 px-4 text-sm font-bold text-white" to="../evidencias">Agregar documentos</Link>} /></> : <>
      ...
      <section className="space-y-3"><SectionHeader eyebrow="ANTECEDENTES" title="Documentos ambientales" action={<Link className="text-sm font-bold text-[var(--brand-primary)]" to="../evidencias">Ir a evidencias</Link>} />
        {state.documentsError ? <ErrorState description={state.documentsError} onRetry={load} /> : !state.documents.length ? <EmptyState icon={FileText} title="Sin documentos asociados" description="Todavía no existen documentos ambientales vinculados a esta obra." guidance="Incorpora evidencias para habilitar validaciones y alertas de vigencia." primaryAction={<Link className="inline-flex min-h-11 items-center rounded-xl bg-emerald-700 px-4 text-sm font-bold text-white" to="../evidencias">Ir a evidencias</Link>} /> :
