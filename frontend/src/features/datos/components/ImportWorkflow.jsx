...
export default function ImportWorkflow({ organizationId, onCompleted, onClose }) {
  ...
  const canConfirm = can("imports.confirm");
  ...
  return <Card><CardContent>
    ...
    {stage === 5 && preview && <section className="mt-5 space-y-4"><h3 className="font-black">Confirmar importación</h3><Alert tone="info" title="Efecto de la confirmación">Se intentarán incorporar {preview.filas_validas} filas válidas a {destinationLabel(destination)}{selectedWork?.nombre ? ` de ${selectedWork.nombre}` : " de la organización"}. {preview.filas_problematicas} filas presentan observaciones y permanecerán trazables.</Alert>{!canConfirm && <Alert tone="warning" title="Revisión requerida">Tu rol puede preparar y revisar la importación, pero la confirmación requiere una capacidad adicional.</Alert>}<div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setStage(4)}>Volver a revisar</Button>{canConfirm && <Button disabled={busy || preview.filas_validas === 0} loading={busy} onClick={confirm}>Confirmar importación</Button>}</div></section>}
