...
function EvidenciasPage() {
  ...
  async function loadData(nextFilters = filters) {
    ...
    try {
      ...
      const [evidenciasData, kpisData] = await Promise.all([
        getEvidenciasConstructora(activeConstructoraId, nextFilters),
        getEvidenciasKpisConstructora(activeConstructoraId),
      ]);
  ...
  function openImportModal(item) {
    setImportModalContext({
      ...
      initialObraId: item.obra_codigo || "",
    });
  ...
  function validateForm() {
    if (!form.nombre.trim() || !form.tipo_evidencia || !form.archivo) {
      return "Nombre, tipo de evidencia y archivo son obligatorios.";
    }
    if (form.alcance === "unidad" && !form.etapa_id.trim()) {
  ...
  async function onSubmit(event) {
    ...
    try {
      ...
      if (form.alcance === "unidad") formData.append("etapa_id", form.etapa_id.trim());
      if (form.alcance === "obra") formData.append("obra_id", form.obra_id.trim());
      if (form.alcance === "emision") formData.append("emision_id", form.emision_id.trim());
      if (form.alcance === "transporte" && form.obra_id.trim()) formData.append("obra_id", form.obra_id.trim());
      if (form.fecha_evidencia) formData.append("fecha_evidencia", form.fecha_evidencia);
    ...
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-10 lg:py-12">
      <section className="rounded-3xl border border-[#B7DEC9] bg-[var(--success-bg)] p-5 shadow-[0_18px_45px_var(--shadow)] sm:p-7">
        <div className="grid gap-6 lg:grid-cols-[1.35fr_0.75fr] lg:items-center">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            <KpiImpact icon={<FileText size={22} />} label="Evidencias totales" value={formatNumber(kpis?.total_evidencias || 0, 0)} detail="Evidencias de respaldo" tone="emerald" />
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-[0_14px_35px_var(--shadow)] sm:p-6">
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_180px_180px_180px_auto]">
          <input
            ...
            placeholder="Buscar evidencia, obra, etapa o registro"
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <form onSubmit={onSubmit} className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-[0_18px_45px_var(--shadow)] sm:p-6">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <select className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)]" value={form.tipo_evidencia} onChange={(event) => setForm((current) => ({ ...current, tipo_evidencia: event.target.value }))}>
              {constructionEvidenceTypeOptions.map(([tipo, label]) => <option key={tipo} value={tipo}>{label}</option>)}
            </select>
            ...
            {form.alcance === "unidad" && <input className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)] md:col-span-2" placeholder="Etapa / frente" value={form.etapa_id} onChange={(event) => setForm((current) => ({ ...current, etapa_id: event.target.value }))} />}
            {form.alcance === "obra" && <input className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)] md:col-span-2" placeholder="Obra asociada" value={form.obra_id} onChange={(event) => setForm((current) => ({ ...current, obra_id: event.target.value }))} />}
            {form.alcance === "emision" && <input className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)] md:col-span-2" placeholder="Registro de emision asociado" value={form.emision_id} onChange={(event) => setForm((current) => ({ ...current, emision_id: event.target.value }))} />}
            {form.alcance === "transporte" && <input className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)] md:col-span-2" placeholder="Obra asociada opcional" value={form.obra_id} onChange={(event) => setForm((current) => ({ ...current, obra_id: event.target.value }))} />}
            <input type="date" className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3 text-sm text-[var(--text-main)]" value={form.fecha_evidencia} onChange={(event) => setForm((current) => ({ ...current, fecha_evidencia: event.target.value }))} />
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-[0_18px_45px_var(--shadow)]">
          ...
          <table className="min-w-[1500px] w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-left text-xs uppercase tracking-wide text-[var(--text-muted)]">
                ...
                <th className="px-3 py-3">Fecha evidencia</th>
              ...
              {visibleRows.map((item) => (
                <tr key={item.id} className="border-b border-[#C9D6CF] text-[#1F2937] hover:bg-[var(--bg-surface)]">
                  ...
                  <td className="px-3 py-3 text-[#475467]">
                    {formatEvidenceType(item.tipo_evidencia)}
                  </td>
                  <td className="px-3 py-3">
                    <div className="space-y-1">
                      <p className="font-semibold text-[var(--text-main)]">{item.obra_codigo || item.Constructora_codigo || "Obra asociada"}</p>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {item.archivo_url ? <button type="button" onClick={() => openImportModal(item)} className="rounded-full border border-[#B7DEC9] bg-[var(--success-bg)] px-3 py-1 text-xs font-bold text-[var(--primary-dark)]">Analizar evidencia</button> : null}
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <ImportarEvidenciaObraModal
          ...
          initialObraId={importModalContext?.initialObraId || ""}
          onClose={() => {
