...
export default function MaterialsPage() {
  ...
  const balances = resourceData(operation.materials, []).flatMap((material) => (material.balances || []).map((balance) => ({ ...balance, materialId: material.material_id, signals: material.senales || [] })));
  ...
  return <div className="space-y-6">
    ...
    {!balancesReady ? <ErrorState title="No fue posible cargar el balance de materiales" description="Los eventos continúan disponibles si pudieron cargarse." /> : <>{signals.length > 0 && <Alert tone="warning" title="Datos por revisar">{signals.length} señales del balance requieren revisión. No se utilizó cantidad inicial como autoridad.</Alert>}<section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{summary.map((item) => <KpiCard key={item.label} label={item.label} value={item.value} unit={item.unit} helper={item.value === null ? "No calculable en una unidad única" : undefined} />)}</section></>}
    {!eventsReady ? <ErrorState title="No fue posible cargar los eventos de materiales" description="El balance continúa disponible si pudo calcularse." /> : !events.length ? <EmptyState title="No hay eventos de materiales para esta obra" description="La carga documental permitirá reconstruir recepción, uso, devolución, reutilización y residuo." primaryAction={<Link className="font-bold text-[var(--brand-primary)]" to="/datos/evidencias">Agregar evidencia</Link>} /> : <section><SectionHeader title="Lineage de materiales" description="Evento y referencia de origen, sin construir un grafo artificial." count={events.length} />
