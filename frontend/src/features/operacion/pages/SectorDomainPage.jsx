...
import { additiveMetrics, DOMAIN_CONFIG, domainMetrics, domainRecords, nonAdditiveMetrics, recordMeasurements } from "../utils/operationSelectors";
...
export default function SectorDomainPage({ domain }) {
  ...
  const records = domainRecords(operation.records, domain);
  ...
  const pointNames = new Map((operation.points || []).map((point) => [String(point.id), point.nombre]));
