...
import { Alert, EmptyState, KpiCard, SectionHeader, TableBody, TableCell, TableHead, TableShell } from "@/shared/ui";
import { formatDateTime, formatNumber } from "@/shared/utils/formatters";
import { transportMetrics } from "../utils/operationSelectors";
import DomainApplicability from "../components/DomainApplicability";

export default function TransportPage() {
  const { obraId } = useParams();
  const { context, operation } = useOutletContext();
  const metrics = transportMetrics(operation.transport);
  const journeys = operation.journeys || [];
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{metrics.map((metric) => <KpiCard key={metric.key} label={metric.label} value={metric.value} unit={metric.unit} />)}</section>
    {(operation.transport?.oportunidades || []).length > 0 && <Alert tone="warning" title="Atención requerida">{operation.transport.oportunidades.map((item) => <p key={`${item.tipo}-${item.ruta || "general"}`}>{item.tipo.replaceAll("_", " ")}: {formatNumber(item.valor)} {item.unidad}</p>)}</Alert>}
    {!journeys.length ? <EmptyState title="No hay viajes registrados para esta obra" description="Importa datos o agrega evidencia logística para comenzar el seguimiento." primaryAction={<Link className="font-bold text-[var(--brand-primary)]" to="/datos/importaciones">Importar datos</Link>} /> : <section><SectionHeader title="Viajes" count={journeys.length} />
