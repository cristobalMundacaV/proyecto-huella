...
function ActivityForm({value,setValue,data,onSave}) { const set=(field)=>(e)=>setValue({...value,[field]:e.target.value}); return <div className="grid gap-3 md:grid-cols-2"><Input label="Código estable" value={value.codigo} onChange={set("codigo")}/><Input label="Nombre" value={value.nombre} onChange={set("nombre")}/><Select label="Tipo" value={value.tipo} onChange={set("tipo")}>{["transporte","consumo_energia","consumo_agua","consumo_combustible","operacion_maquinaria","movimiento_material","gestion_residuo","generacion_energia","proceso_productivo","otro"].map(x=><option key={x}>{x}</option>)}</Select><Input label="Inicio" type="datetime-local" value={value.timestamp_inicio} onChange={set("timestamp_inicio")}/><Select label="Unidad" value={value.unidad_operacional} onChange={set("unidad_operacional")}><option value="">Sin unidad</option>{data.unidades.map(x=><option key={x.id} value={x.id}>{x.nombre}</option>)}</Select><Select label="Proceso" value={value.proceso_operacional} onChange={set("proceso_operacional")}><option value="">Sin proceso</option>{data.procesos.map(x=><option key={x.id} value={x.id}>{x.nombre}</option>)}</Select><div><Save onClick={onSave}/></div></div> }
