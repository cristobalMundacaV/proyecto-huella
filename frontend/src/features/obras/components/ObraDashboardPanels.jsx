...
export function AiRecommendationsPanel({ recommendations = [] }) {
  ...
  return <Card className="border-slate-200 bg-white/90"><CardContent>
    ...
    {!visible.length ? <p className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">El motor de diagnóstico no identificó recomendaciones prioritarias con la información disponible.</p> : <div className="grid items-start gap-3 lg:grid-cols-3">{visible.map((item) => <article className="relative flex flex-col rounded-[18px] border border-slate-200 bg-[linear-gradient(145deg,#fff,#f8fafc)] p-4 shadow-[0_7px_20px_rgba(15,23,42,.04)]" key={item.key}>
      ...
      <Link className="ml-auto mt-4 inline-flex items-center gap-1 text-xs font-black text-emerald-800" to={item.href}>Revisar <ArrowRight size={13} /></Link>
    </article>)}</div>}
  </CardContent></Card>;
}
