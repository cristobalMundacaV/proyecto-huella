...
import { AiRecommendationsPanel } from "@/features/obras/components/ObraDashboardPanels";
import EnvironmentalBalanceSection from "@/features/obras/components/EnvironmentalBalanceSection";
import { buildFlowImpactDonutData, buildReadinessRings, buildRecommendations, buildScopeDonutData, describeEmissionState } from "@/features/obras/utils/obraDashboardSelectors";
import { buildEnvironmentalBalance, buildGeiPerformance, buildMultiFlowNarrative } from "@/features/obras/utils/environmentalPerformance";
...
export default function ObraResumenPage() {
  const { obra, dashboard, dashboardError, operation } = useOutletContext();
  ...
  return <WorkExecutiveDashboard dashboard={dashboard} obra={obra} operation={operation} />;
}

export function WorkExecutiveDashboard({ dashboard, obra, operation }) {
  ...
  const balance = buildEnvironmentalBalance({
    records: resourceData(operation?.records, []),
    journeys: resourceData(operation?.journeys, []),
    materials: resourceData(operation?.materials, []),
  });
  const gei = buildGeiPerformance(dashboard);
  const recommendations = buildRecommendations(dashboard);
  const reading = buildMultiFlowNarrative(balance);
  ...
  return <div className="space-y-3 pb-4">
    ...
    <section className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5" aria-label="Estado general del período">
      <CZMetricCard icon={Cloud} label="Estado ambiental" value={emissionState.label} supportingText={emissionState.helper} tone={emissionState.tone} />
      <CZMetricCard icon={ShieldCheck} label="Cobertura de datos" value={readiness} unit="%" supportingText={dashboard.readiness?.listo_para_reporte ? "Listo para reporte" : "Período no listo para cierre"} tone="emerald" progress={readiness} />
      <CZMetricCard icon={FileCheck2} label="Cobertura de evidencia" value={evidence} unit="%" supportingText={`${dashboard.resumen_ejecutivo?.evidencias_faltantes ?? 0} evidencias faltantes`} tone="rose" progress={evidence} />
      <CZMetricCard icon={AlertTriangle} label="Hallazgos críticos" value={critical} supportingText={critical ? "Requieren atención" : "Sin hallazgos altos"} tone="rose" />
      <CZMetricCard icon={AlertTriangle} label="Riesgo ambiental" value={risk} supportingText={dashboard.risk?.nivel ? `Nivel ${String(dashboard.risk.nivel).replaceAll("_", " ")}` : "Sin evaluación"} tone="orange" />
    </section>

    <EnvironmentalBalanceSection flows={balance} obraId={obra.id} />

    <section className="rounded-[22px] border border-slate-200 bg-white/90 p-4 shadow-[0_10px_30px_rgba(15,23,42,.05)]" id="readiness">
      <div className="mb-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-700">Cambio climático</p><h2 className="text-xl font-bold text-slate-950">Clima / GEI</h2><p className="mt-1 text-sm font-normal text-slate-500">{gei.scopeCaveat}</p></div>
      <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
        <CZMetricCard icon={Leaf} label="Huella total" value={gei.total} unit="tCO2e" tone="blue" />
        <CZMetricCard icon={Leaf} label="Alcance 1" value={gei.scope1} unit="tCO2e" supportingText={share(gei.scope1, gei.total)} tone="emerald" />
        <CZMetricCard icon={Zap} label="Alcance 2" value={gei.scope2} unit="tCO2e" supportingText={share(gei.scope2, gei.total)} tone="amber" />
        <CZMetricCard icon={Layers3} label="Alcance 3" value={gei.scope3} unit="tCO2e" supportingText={share(gei.scope3, gei.total)} tone="violet" />
      </div>
      <div className="mt-3 grid gap-3 xl:grid-cols-[1fr_1fr_.9fr]">
        <ChartCard title="GEI por alcance" description="Distribución de la huella total entre Alcance 1, 2 y 3." empty={!scopeData.some((row) => Number(row.value) > 0)}><DonutPanel data={scopeData} total={gei.total} /></ChartCard>
        <ChartCard title="GEI por flujo" description="Contribución de cada flujo a la huella de carbono (kgCO2e → tCO2e), no su desempeño físico." empty={!flowData.length}><DonutPanel data={flowData} total={gei.total} /></ChartCard>
        <ChartCard title="Calidad y trazabilidad" description="Cobertura, evidencias, factores y validación profesional."><div className="grid min-h-[300px] content-center gap-2 sm:grid-cols-2 xl:grid-cols-1">{rings.map((ring) => <div className="rounded-xl bg-slate-50 px-3 py-2" key={ring.key}><CoverageProgressChart label={ring.label} value={ring.value} size={54} strokeWidth={7} valueClassName="text-sm text-[var(--text-primary)]" /></div>)}</div></ChartCard>
      </div>
    </section>
