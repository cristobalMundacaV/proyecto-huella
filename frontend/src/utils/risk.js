...
export function calculateRiskProfile(data, optimizedScenario) {
  ...
  const dieselEntry = Object.entries(data?.emisiones_por_actividad || {}).find(
    ([activity]) => activity.toLowerCase() === "diesel"
  );
  const dieselPresent = Boolean(dieselEntry && Number(dieselEntry[1]) > 0);
